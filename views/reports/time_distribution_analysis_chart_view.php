    ...
    <head>
		...
		<script type="text/javascript">
			...
			function valueChange(){
					...
					if(reportArray.length == 2){
						if( obj.user == reportData.user && obj.startDate == reportData.startDate) {
							$("#canvas,#chart_type,#report_title").css("visibility", "visible");
			...
			function generateReport(){
				...
				$.ajax({
					...
					success: function(data){
                        ...
                        if(data.hasOwnProperty('arraydata')){
						  ...
						  $("#report_title").css({"visibility": "visible","display": "block"}).html("<center>No records available for the selected time duration.</center><hr>");
                            return;
						}
			...
			function buildHtmlTable(myList,selector, inputdata, state) {
				myList = myList.arraydata
				format(myList,inputdata, state)
				
				if(state == "Saved"){
					$("#report_title").html(state+" Analysis for <u>"+ inputdata.name +"</u>  On <u>"+ moment($("#start_date input").val(), 'DD-MM-YYYY').format(dateFormat) +"</u><hr>");
				}else{
					$("#report_title").html(state+" Analysis for <u>"+ inputdata.name +"</u>  On <u>"+ moment(inputdata.startDate, "DD-MM-YYYY").format(dateFormat) +"</u><hr>");
				}
			...
			<script type="text/javascript">
				  ...
				  function format(data , inputdata , state){
						...
						for (var j = 0; j < data.length; j++) {
							total += moment(data[j]["Activity_TimeDuration"],"HH.mm.ss").diff(moment().startOf('day'), 'seconds');
						}
						...
						for (var i = 0; i < data.length; i++) {
							...
							var sec = moment(rowHash["Activity_TimeDuration"],"HH.mm.ss").diff(moment().startOf('day'), 'seconds');
							//sec += moment(rowHash["Activity_TimeDuration"],"HH.mm.ss").diff(moment().startOf('day'), 'seconds');
							var time = moment(rowHash["Activity_TimeDuration"] , "HH:mm:ss");
							var dutation = time.format("HH") +" Hrs "+time.format("mm")+" Min "+ time.format("ss") +" Sec";
							/* var dutation = "dutation : " +time.format("HH") +" Hrs "+time.format("mm")+" Min "+ time.format("ss") +" Sec"; */
							var summary = 	"Title : "+columnSet1[0]+
											"\n Duration : " +time.format("HH") +" Hrs "+
											time.format("mm")+" Min "+
											time.format("ss")+" Sec" +
											/* "\n Time in sec : "+ sec+ */
