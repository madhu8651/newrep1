		...
		<script type="text/javascript">
			...
			function savedreportdisplay(){
				if(reportArray.length == 3 ){
                    ...
                    obj1.type='Activity_Cost_Distribution';
					...
					$.ajax({
						...
						success: function(data){						
							...
							reportData.type = 'Activity_Cost_Distribution';
							...
							data_req.type='Activity_Cost_Distribution';
							if(data.lid){
								buildHtmlTable(data.tabledetails,"#excelDataTable" , reportData, "Detailed");
							}else{								
			...
			function generateReport(){
				if($("#sub_user").val() == ""){
					$("#sub_user").next(".error-alert").text("Select an lead.");
					return;
				}else{
				...
				obj.type='Activity_Cost_Distribution';
				obj.subtype='Summary';
				$.ajax({
					type: "POST",
					...
					success: function(data){
						...
						for(i = 0; i < data.length; i++){
							if(data[i].Total_Activity_Cost == "0.00"){
								count = 1;
							}
						}
						if(data.length > 0 && count != 1){
							...
							if(count == 1){
								...
								$("#report_title").css({"visibility": "visible","display": "block"}).html("<center>No activity cost defined</center>");
							}else{
			...
			function buildHtmlTable(myList,selector, inputdata, state) {
				...
				if(state == "Last viewed"){
					...
					if(inputdata.startdate == 'NA'){
                     $("#report_title").html("<span>"+state+" Analysis for Lead <u><i>"+ name +"</i></u></span>");
				    }else{
						var total = getCookie("total");
						if(total == 0.00 && myList[0].hasOwnProperty('Total_Activity_Cost')){
							$("#report_title").html("<center>No activity cost defined</center>");
						}else{
							$("#report_title").html("<span>"+state+" Analysis for Lead <u><i>"+ name +"</i></u>  From <u>"+ moment(inputdata.startdate, 'DD-MM-YYYY').format(dateFormat) +"</u>  To <u>"+ moment(inputdata.enddate, 'DD-MM-YYYY').format(dateFormat) +"</u></span>");
						}                       
				    }
				}else{
                    if(inputdata.startdate == 'NA'){
                       $("#report_title").html(state+" Analysis for Lead <u><i>"+ name +"</i></u>");
                    }else{
                          $("#report_title").html("<span>"+state+" Analysis for Lead <u><i>"+ name +"</i></u>  From <u>"+ moment(inputdata.startdate, 'DD-MM-YYYY').format(dateFormat) +"</u>  To <u>"+ moment(inputdata.enddate, 'DD-MM-YYYY').format(dateFormat) +"</u></span>");
    		        }
				}
				
                if(reportData.lid == "All"){
                    if(inputdata.startdate == 'NA'){
                       $("#report_title").html("<span>"+state+" Analysis for Lead <u><i>"+name+"</i></u> (All Activities)</span>");
                    }else{
                         $("#report_title").html("<span>"+state+" Analysis for Lead <u><i>"+name+"</i></u>  (All Activities) From <u>"+ moment(inputdata.startdate, 'DD-MM-YYYY').format(dateFormat) +"</u>  To <u>"+ moment(inputdata.enddate, 'DD-MM-YYYY').format(dateFormat) +"</u></span>");
    		        }
                }else if (typeof reportData.lid != 'undefined'){
					if(inputdata.startdate == 'NA'){
                       $("#report_title").html(state+" Analysis for Lead <u>"+ name +"</u> ("+reportData.actname+")");
                    }else{
                         $("#report_title").html(state+" Analysis for Lead <u>"+ name +"</u> ("+reportData.actname+") From <u>"+ moment(inputdata.startdate, 'DD-MM-YYYY').format(dateFormat) +"</u>  To <u>"+ moment(inputdata.enddate, 'DD-MM-YYYY').format(dateFormat) +"</u>");
    		        }
				}
			...
			<script type="text/javascript">
				function format(data , inputdata , state){
						...
						for(i = 0; i < columnSet.length; i++){
							total = total + parseFloat(columnSet[i][2]);
						}
						total = total.toFixed(2);
						if(total == '0.00'){
							$('#report_title').append('');
						}else{
							$('#report_title').append('<span><b><h3 style="display: inline;"> & </h3> <i>the total cost is INR.'+total+'</i></b></span>');
						}
                        setCookie('total', total,1);

						for(j=0;j<columnSet.length;j++){
							data.addRow([columnSet[j][0], 
							parseInt(columnSet[j][2]), 
							columnSet[j][0] + " Rs " + columnSet[j][2] + "\n Total Cost Rs " + total , 
							columnSet[j][1]]);
						}
						var report_title =state+" Analysis for "+ inputdata.lname +" On "+ moment(inputdata.startdate, "DD-MM-YYYY").format(dateFormat);
						...
						var chart = new google.visualization.PieChart(document.getElementById('piechart_3d'));
							...
							var selectHandler = function(e) { 
								...
								if (selectedItem) {
									var actname = data.getValue(selectedItem.row, 2). split('Rs');
									reportData["actname"] = actname[0];
				...
				function additionfun(data){
                      var Resource_Cost_total=0,Activity_Cost_total=0, Total_Activity_Cost_total=0;
    				  for(i=0; i< data.length; i++){
    				       Resource_Cost_total = Resource_Cost_total + parseFloat(data[i].Resource_Cost);
                                 Activity_Cost_total =   Activity_Cost_total +  parseFloat(data[i].Activity_Cost);
                                 Total_Activity_Cost_total =  Total_Activity_Cost_total + parseFloat(data[i].Total_Cost);
    				  }
                            var addition = {};
                            var addition1 = '';
                            addition1 = '{';
                            for (var key in data[0]) {
        					    if ($.inArray(key, addition) == -1) {
        					      if(key == 'Resource_Cost'){
									addition1 += '"'+key +'":'+ '"<b>Total - '+Resource_Cost_total.toFixed(2)+'</b>",';
        					      }else if(key == 'Activity_Cost'){
									  addition1 += '"'+key +'":'+ '"<b>Total - '+Activity_Cost_total.toFixed(2)+'</b>",';
        					      }else if(key == 'Total_Cost'){
									 addition1 += '"'+key +'":'+'"<b>Total - '+Total_Activity_Cost_total.toFixed(2)+'</b>",';
        					      }else if(key == 'Remarks'){
                                      addition1 += '"'+key +'":'+ '""';
        					      }else if(key == 'Media_File'){
                                      addition1 += '"'+key +'":'+ '"\'blank\'" ,';
        					      }else{
                                    addition1 += '"'+key +'":'+ '"" ,';
        					      }
