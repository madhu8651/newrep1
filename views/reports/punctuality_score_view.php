...
<html lang="en">
    <head>
		...
		<script type="text/javascript">
			...
			function valueChange(cookie){
					var obj={};
					obj.team = $("#team").val();
					obj.location = $("#off_location").val();
					obj.resource = $("#resource_name").val();
					...
					if(reportArray.length == 3 ){
						...
						if( obj.resource == reportData.resource && obj.location == reportData.location &&
						obj.team == reportData.team &&	obj.startDate == reportData.startDate && obj.endDate == reportData.endDate) {
							changedInput=0;
				...
				if(reportArray.length == 3 ){
					...
					$.ajax({
						...
						success: function(data){
							...
							reportData={};
							reportData.team = data.team;
							reportData.location = data.location;
							reportData.resource = data.resource;
							...
							$("#team").val(reportData.team);
							$("#off_location").val(reportData.location);
							$("#resource_name").val(reportData.resource);
			...
			------------------------------------------------------------------------------------------------------------ */
            function list_view(filter, cookie){
				valueChange(cookie);
                team = $("#team").val() !='' ? $("#team").val() : '';
                off_location = $("#off_location").val() !='' ? $("#off_location").val() : '';
                data={};
                data.filter=filter;
                data.team=team;
                data.off_location=off_location;
				$.ajax({
					type: "POST",
					url:"<?php echo site_url('reports/standard_RepAnalysisController/fetch_filter_data')?>",
					dataType:'json',
					...
					success: function(data){
						...
						var select = $("#"+filter), options = "<option value=''>Select</option><option value='All'>All";
						select.empty();
						for(var i=0;i<data.length; i++){
							options += "<option value='"+data[i].dataid+"'>"+ data[i].dataname+"</option>";
						...
						if(cookie){
							...
							$("#team option[value='"+prviouse_data.team+"']").attr("selected", true);
							officeLocation('off_location', 'cookie');
						}
						loaderHide();
					},
					error:function(data){
			...
			function officeLocation(filter, cookie){
				valueChange(cookie);
				if(cookie){
					var prviouse_data =  $.parseJSON(getCookie("reportInput"));
					team = prviouse_data.team;
					off_location = prviouse_data.location;
				}else{
					team = $("#team").val() !='' ? $("#team").val() : '';
					off_location = $("#off_location").val() !='' ? $("#off_location").val() : '';
				}
				...
				$.ajax({
					...
					success: function(data){
						...
						if(cookie){
							var prviouse_data =  $.parseJSON(getCookie("reportInput"));
							$("#off_location option[value='"+prviouse_data.location+"']").attr("selected", true);
							resource('resource_name', 'cookie');
						}
						loaderHide();
					},
					error:function(data){
			...
			function resource(filter, cookie){
				if(cookie){
					var prviouse_data =  $.parseJSON(getCookie("reportInput"));
					team = prviouse_data.team;
					off_location = prviouse_data.location;
				...
				$.ajax({
					...
					success: function(data){
						...
						if(cookie){
							var prviouse_data =  $.parseJSON(getCookie("reportInput"));
							$("#resource_name option[value='"+prviouse_data.resource+"']").attr("selected", true);
							$('#start_date input').val(prviouse_data.startDate);
							$('#endDate input').val(prviouse_data.endDate);
						}
						valueChange(cookie);
						savedreportdisplay();
						loaderHide();
            ...
            /****** Validation ****************/
            function validate(fieldid,errormsg,fieldid1){
               if($("#"+fieldid).val()==""){
                  	$("#"+fieldid1).next(".error-alert").text(errormsg);
					return 1;
               }else{
                    $("#"+fieldid1).next(".error-alert").text("");
               }
            }
			/* --------------------------------------------------------------------------------------------------------
			...
			function generateReport(){
                ...
                var flag=validate('team','Select a team','team');
                if(flag==1){ return; }
                var flag=validate('off_location','Select a Office location','off_location');
                if(flag==1){ return; }
                var flag=validate('resource_name','Select a Resource Name','resource_name');
                if(flag==1){ return; }
                var flag=validate("start_date input",'Select a Start date','start_date');
                if(flag==1){ return; }
                var flag=validate("end_date input",'Select a End date','end_date');
                if(flag==1){ return; }
			...
			function buildHtmlTable(myList,selector, inputdata, state) {
				console.log(myList)
				if(state == "Saved"){
					$("#report_title").html(state+" Analysis  form "+ moment($("#start_date input").val(), 'DD-MM-YYYY').format(dateFormat) +"</u> to <u>"+moment($("#end_date input").val(), 'DD-MM-YYYY').format(dateFormat)+"</u><hr>");
				}else{
					$("#report_title").html(state+" Analysis  form "+ moment(inputdata.startDate, "DD-MM-YYYY").format(dateFormat) +"</u> to <u>"+ moment(inputdata.endDate, "DD-MM-YYYY").format(dateFormat) +"</u><hr>");
				}
				...
				if(reportArray.length == 2 ){
					...
					if(state == "Last viewed"){
						$("#canvas, #report_title").css({"visibility": "visible","display": "block"});
					}
				}
				if(reportArray.length == 3 )
                {
			...
			function save(state){
				...
				$.ajax({
					type: "POST",
					url:"<?php echo site_url('reports/standard_RepAnalysisController/save_report/Punctuality_Score')?>",
				...
				<div class="page-container">
					<div class="row report_input_data" >
						...
						<div class="col-md-2">
							...
							<select class="form-control" id="team" onchange="officeLocation('off_location')">
							</select>
							<span class="error-alert"></span>
						</div>
                        <div class="col-md-2">
							<label for="off_location">Location</label>
							<select class="form-control" id="off_location" onchange="resource('resource_name')">
							</select>
							<span class="error-alert"></span>
						</div>
                        <div class="col-md-2">
							<label for="resource_name">Resource Name</label>
							<select class="form-control" id="resource_name" onchange="valueChange()">
							</select>
							<span class="error-alert"></span>
						</div>
							<div class="col-md-3">
