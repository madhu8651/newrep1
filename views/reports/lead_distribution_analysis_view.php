		...
		<script type="text/javascript">
			...
			$(document).ready(function(){
				...
				if(getCookie("reportInput") != ''){
					...
					$("#sub_user option[value='"+prviouse_data.category+"']").attr("selected", true);
			...
			function generateReport(){
				if($("#sub_user").val() == ""){
					$("#sub_user").next(".error-alert").text("Select a category.");
					return;
				}else{
				...
				obj.type='Lead_Distribution_Analysis';				
				obj.subtype='NA';
				$.ajax({
					type: "POST",
					...
					success: function(data){
						...
						if(data.length > 0){
							...
							$("#piechart_3d, #table_view, #chart_select").hide();
						}
					},
					error:function(data){
						network_err_alert(data);
			...
			/* generated detail report from summary table view */
			...
			function buildHtmlTable(myList,selector, inputdata, state) {
				...
				if(state == "Saved"){
					$("#report_title").html(state+" Analysis is for <u>"+ inputdata.category +"</u>  From <u>"+ moment($("#start_date input").val(), 'DD-MM-YYYY').format(dateFormat) +"</u>  From <u>"+ moment($("#end_date input").val(), 'DD-MM-YYYY').format(dateFormat) +"</u><hr>");
				}else if(state == "Child Report"){
					state1 = "Child";
					$("#report_title").html(state1+" Analysis is for <u>"+ inputdata.category +"</u>  From <u>"+ moment(inputdata.startdate, "DD-MM-YYYY").format(dateFormat) +"</u>  To <u>"+ moment(inputdata.enddate, "DD-MM-YYYY").format(dateFormat) +"</u><hr>");
				}else{
					$("#report_title").html(state+" Analysis is for <u>"+ inputdata.category +"</u>  From <u>"+ moment(inputdata.startdate, "DD-MM-YYYY").format(dateFormat) +"</u>  To <u>"+ moment(inputdata.enddate, "DD-MM-YYYY").format(dateFormat) +"</u><hr>");
				}
				...
				/* if(state != "Child Report" || inputdata.subtype != "details"){ */
				...
				export_report('lead_distribution_analysis_view');
			}
				...
				$(selector).html("")
					...
					for (var i = 0; i < myList.length; i++) {
						...
						console.log(columns[colIndex])
							...
							/* --------------------------------- */
							if(columns[colIndex] == 'Lead_Create_Date'){
				  ...
				  function format(data, inputdata, state, selector){
						...
						*/
						...
						var selectHandler = function(e){
							...
							if (selectedItem){
								if(reportData.hasOwnProperty('category')== false){
									return;
								}
