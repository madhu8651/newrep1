    ...
    <head>
		...
		<script type="text/javascript">
			...
			function valueChange(){
					...
					obj.user_name = $("#user_list option:selected").text();
					obj.customer = $("#cust_list").val();
					obj.customer_name = $("#cust_list option:selected").text();
					...
					if(reportArray.length == 3 ){
						...
						if( obj.customer_name == reportData.customer_name && obj.user_name == reportData.user_name ){
							changedInput=0;
					...
					if(reportArray.length == 2 ){
						if(	obj.customer== reportData.customer ){
			...
			function savedreportdisplay(){
				if(reportArray.length == 3 ){
                    ...
                    obj1.type='Cus_retentioncostanalysis';
				    obj1.subtype='Summary';

					$.ajax({
						type: "POST",
						url:"<?php echo site_url('reports/standard_CusAnalysisController/getsaved_reports')?>",
						...
						success: function(data){
							...
							reportData.type='Cus_retentioncostanalysis';
							...
							reportData.user = $("#user_list option:selected").val();
							reportData.user_name = $("#user_list option:selected").text();
							reportData.customer = $("#cust_list option:selected").val();
			...
			function generateReport(pos){
				...
				if(pos == 'back'){
					...
					*/
					...
					obj.type='Cus_retentioncostanalysis';
					obj.subtype='Summary';
					...
					var initial={};
					initial.input=obj;
					initial.state = "Generated Report Summary";
					initial.name = 'User: '+user_name+' And Customer: '+customer_name;
			...
			function child_report(id, name, state){
				//if(reportData.hasOwnProperty('startDate') && reportData.hasOwnProperty('endDate') && reportData.hasOwnProperty('subtype')){
					...
					reportData["lookup_id"] = id;
					reportData["lookup_value"] = name;
					reportData["subtype"] = "Details";
					var inti = JSON.parse(getCookie("initial"))
					
					var initial={};
					initial.input = reportData;
					initial.state = "Generated Details Report";
					
					initial.name = inti.name;
					setCookie('initial', JSON.stringify(initial),1);
			...
			function buildHtmlTable(myList,selector, inputdata, state , name) {
				...
				/* ====================================================================================== */
				/* ====================================================================================== */
				console.log(JSON.parse(getCookie("initial")));
				var title = JSON.parse(getCookie("initial"))
				
				$("#report_title").html(title.state+" for <b>"+ title.name +"</b><hr>");/*"+ name +" removed element for referece */
				
				if(myList[0].hasOwnProperty('Contact_Number')){
			...
			function save(state){
				...
				$.ajax({
					type: "POST",
				   	url:"<?php echo site_url('reports/standard_CusAnalysisController/save_report/Cus_retentioncostanalysis')?>",
			...
			<script type="text/javascript">
				...
				function pie_Chart(data , inputdata , state){
					$("#show_all_details").remove();
					$("#canvas").append("<center id='show_all_details' style='margin-bottom:6px'><button class='btn'> Show All Details </button></center>");
					$("#show_all_details button").click(function(){
						child_report('All','All','Details Report');
					})
					...
					for (var i = 0; i < data.length; i++) {
						...
						"\n Cost :  "+(data[i].Activity_Cost / 100)+ " Rs "+
						"\n Percentage : "+ ((data[i].Activity_Cost/total) * 100).toFixed(1)+" %";
					...
					function drawChart(){
						...
						data.addColumn({ type: 'number', id: 'cost' });	
						data.addColumn({'type': 'string', 'role': 'tooltip', 'p': {'Html': true}});							
						data.addColumn({type: 'string', id: 'lookup_id'});
						...
						var options = {
							...
							//title: "<button> All Details</button>", 
							is3D: true,
						...
						//if(reportData.hasOwnProperty('startDate') && reportData.hasOwnProperty('endDate') && reportData.hasOwnProperty('type')){
							var selectHandler = function(e){
								...
								if (selectedItem){
									...
									child_report(topping , data.getValue(selectedItem.row, 0), "Details Report");
								}
								chart.setSelection([]);
							}
			...
			<div class="page-container">
				...
				<div class="row print">
					...
					<div class="col-md-12 no-padding print" id="canvas">
						...
						<div class ="print table_view table-responsive" id="table_view" style="margin-bottom:6px;" >
