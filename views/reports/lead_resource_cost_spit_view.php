		...
		<script type="text/javascript">
			...
			function savedreportdisplay(){
				if(reportArray.length == 3 ){
                    ...
                    obj1.type='Lead_Resource_Cost_Split';
					...
					$.ajax({
						...
						success: function(data){
							...
							reportData.type = 'Lead_Resource_Cost_Split';
							...
							data_req.type='Lead_Resource_Cost_Split';
							if(data.hasOwnProperty('resourceid')){
				...
				$.ajax({
					...
					success: function(data){
						...
						if(getCookie("reportInput") != ''){
							...
							$("#sub_user").val(prviouse_data.lead_id);
			...
			function generateReport(){
				...
				obj.type='Lead_Resource_Cost_Split';
                obj.subtype='Summary';
			...
			function buildHtmlTable(myList,selector, inputdata, state) {
				...
				if(inputdata.hasOwnProperty('resourceid')){
                ...
                if(state == "Last viewed"){
					...
					if(inputdata.startdate == 'NA'){
                     $("#report_title").html(state+" Analysis for Lead <u>"+ inputdata.lead_name +"</u>");
                       ...
                       $("#report_title").html(state+" Analysis for Lead <u>"+ inputdata.lead_name +"</u>  From <u>"+ moment(inputdata.startdate, 'DD-MM-YYYY').format(dateFormat) +"</u>  To <u>"+ moment(inputdata.enddate, 'DD-MM-YYYY').format(dateFormat) +"</u>");
				    }
				}else{
                    if(inputdata.startdate == 'NA'){
                ...
                if(inputdata.hasOwnProperty('resourceid')){
					if(inputdata.resourceName == "All"){
						if(inputdata.startdate == 'NA'){
							   $("#report_title").html(state+" resource cost split Analysis for Lead <u>"+inputdata.lead_name+"</u>");
						}else{
							 $("#report_title").html(state+" resource cost split Analysis for Lead <u>"+inputdata.lead_name+"</u>  From <u>"+ moment(inputdata.startdate, 'DD-MM-YYYY').format(dateFormat) +"</u>  To <u>"+inputdata.enddate +"</u>");
						}
					}else{
						if(inputdata.startdate == 'NA'){
							   $("#report_title").html(state+" resource cost split Analysis of "+inputdata.resourceName+" for Lead <u>"+inputdata.lead_name+"</u>");
						}else{
							 $("#report_title").html(state+" resource cost split Analysis of "+inputdata.resourceName+" for Lead <u>"+inputdata.lead_name+"</u>  From <u>"+ moment(inputdata.startdate, 'DD-MM-YYYY').format(dateFormat) +"</u>  To <u>"+ moment(inputdata.enddate, 'DD-MM-YYYY').format(dateFormat) +"</u>");
						}
					}
			...
			<script type="text/javascript">
				function format(data , inputdata , state){
						...
						for (var i = 0; i < data.length; i++) {
							...
							columnSet1.push(data[i].user_id);
							columnSet1.push(data[i].user_id);
						...
						data.addColumn({ type: 'string', id: 'resourceid' });
                        var total=0;
						
                        for(jj=0;jj<columnSet.length;jj++){
							columnSet[jj][1] = parseFloat(columnSet[jj][1]);
                            total =  total + columnSet[jj][1];
                        }
						/* //na = $('#report_title').find('u').text(); */
                        $('#report_title').append('<span><b><h3 style="display: inline;"> & </h3><i>the total cost is INR.'+total+'</i></b></span>');
						
						for(j=0;j<columnSet.length;j++){
							columnSet[j][2] = 'Resource Name: '+columnSet[j][0].split("_").join(" ")  +"\nCost: Rs " + columnSet[j][1] +' \n Total Cost: Rs '+ total;
							data.addRow([   
								columnSet[j][0].split("_").join(" "),
								columnSet[j][1],
								columnSet[j][2],
								columnSet[j][3],
							]);
						...
						var chart = new google.visualization.PieChart(document.getElementById('piechart_3d'));
							...
							if (selectedItem) {
								...
								details(topping , data.getValue(selectedItem.row, 0));
							}
							chart.setSelection([]);
						}
				...
				function details(resourceid,resourceName) {
				    $('#canvas #details_btn').remove();
                      reportData["resourceName"] = resourceName;
                      reportData["resourceid"] = resourceid;
					  reportData["subtype"] = "details";
