    ...
    <head>
		...
		<script type="text/javascript">
			...
			function valueChange(){
					...
					obj.opportunity = $("#product_list").val();
					obj.selectionId = $("#selection_type").val();
					...
					if(reportArray.length == 2 ){
						if(obj.opportunity == reportData.opportunity && obj.endDate == reportData.endDate && obj.startDate == reportData.startDate && obj.selectionId == reportData.selectionId){
							$("#canvas,#chart_type,#report_title").css("visibility", "visible");
			...
			function savedreportdisplay(){
				if(reportArray.length == 3 ){
                    ...
                    obj1.type='opp_performanceanalysis';
					...
					$.ajax({
						...
						success: function(data){
							...
							reportData.selectionId = data.selectionId;
							...
							reportData.type='opp_performanceanalysis';

							$(".pageHeader1 h2").text(reportData.report_name);

							$("#product_list").val(data.opportunity);
							$("#selection_type").val(data.selectionId);
				...
				$.ajax({
					...
					success: function(data){
						...
						if(getCookie("reportInput") !=""){
							var prviouse_data = JSON.parse(getCookie("reportInput"));
							$("#product_list option[value="+prviouse_data.opportunity+"]").prop("selected", true);
							if(prviouse_data.opportunity != "All"){
								$("#selection_type").closest('.col-md-3').fadeOut();
							}else{
								$("#selection_type").closest('.col-md-3').fadeIn();
								$("#selection_type option[value="+prviouse_data.selectionId+"]").prop("selected", true);
							}
						...
						/* --------------------------------------------------------------------- */
							...
							if(reportArray.length == 2 ){
								if (getCookie("reportOutput") != "" && getCookie("reportInput") !="") {
									...
									name = $("#product_list option[value='"+reportInput.opportunity+"']").text();
			...
			function generateReport(pos){
				...
				if(pos == 'back'){
					...
					$("#product_list").val(initial.input.opportunity);
					if(initial.input.opportunity == 'All'){
						$("#selection_type").closest('.col-md-3').fadeIn();
					}else{
						$("#selection_type").closest('.col-md-3').fadeOut();
					}
					...
					if($("#product_list").val() == ""){
						$("#product_list").next(".error-alert").text("Select an Opportunity.");
						return;
					}else{
					...
					obj.type='opp_performanceanalysis';
					obj.subtype='Summary';
					...
					/* -====================================== */
						...
						var initial={};
						var init = {};
						init.opportunity = $("#product_list").val();
						if(obj.opportunity == 'All'){
							init.selectionId = $("#selection_type").val();
						}else{
							init.selectionId = '';
						}
						...
						init.type='opp_performanceanalysis';
			...
			function child_report(id, name, state){
				if(reportData.hasOwnProperty('startDate') && reportData.hasOwnProperty('endDate') && reportData.hasOwnProperty('subtype')){
					...
					reportData["opportunity"] = id;
					reportData["selectionId"] = name;
					reportData["subtype"] = "Details";
					$("#selection_type").closest('.col-md-3').fadeOut();
					$("#product_list").val(reportData.opportunity);
			...
			function buildHtmlTable(myList,selector, inputdata, state , name) {
				...
				/* ====================================================================================== */
				type = $("#product_list option[value='"+inputdata.opportunity+"']").text();
				var stateType = '';
				if(state == 'Last viewed Report'){
					stateType = 'Last viewed Analysis';
				}else if(state == 'Generated Report'){
					stateType = 'Generated Analysis';
				}else if(state == 'First Level Report'){
					stateType = 'Detailed Analysis';
				}else if(state == 'Saved Report'){
					stateType = 'Saved Analysis';
				}
				if(inputdata.selectionId == undefined){
					inputdata.selectionId = '';
				}else{
					inputdata.selectionId = inputdata.selectionId;
				}
				$("#report_title").html(stateType+" for <b>"+ type +' '+ inputdata.selectionId+"</b> From <u>"+ moment(inputdata.startDate, "DD-MM-YYYY").format(dateFormat) +"</u> To <u>"+ moment(inputdata.endDate, "DD-MM-YYYY").format(dateFormat) +"</u>");
			...
			function table_built(myList,selector, inputdata, state){
				...
				for (var c = 0; c < columns.length; c++) {
					if(columns[c] == "Note"){
						...
						if(columns[c] != "opportunity_product" && columns[c] != "opportunity_id"){
				...
				for (var i = 0; i < myList.length; i++) {
					...
					for (var colIndex = 0; colIndex < columns.length; colIndex++) {
						...
						/* --------------------------------- */
						if(columns[colIndex] == 'Opportunity_Date'){
							cellValue = moment(cellValue, 'DD-MM-YYYY').format(dateFormat);
						}
						if(columns[colIndex] != "opportunity_product" && columns[colIndex] != "opportunity_id"){
					...
					if(state == "Last viewed Report" || state == "First Level Report" || myList[0].hasOwnProperty('Contact_Number')){
						...
						$(selector +" tbody").append('<tr onclick="child_report(\''+myList[i].opportunity_id+'\', \''+inputdata.selectionId+'\',\'First Level Report\')">'+datacell+'</tr>');
