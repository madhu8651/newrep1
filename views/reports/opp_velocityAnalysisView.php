...
<html lang="en">
    <head>
		...
		<script type="text/javascript">
			...
			------------------------------------------------------------------------------------------------------------ */
            function list_view(filter){
                data={};
                product = $("#product").val() !='' ? $("#product").val() : '';
                industry = $("#industry").val() !='' ? $("#industry").val() : '';
                bus_location = $("#bus_location").val() !='' ? $("#bus_location").val() : '';
                data.filter=filter;
                data.product=product;
                data.industry=industry;
                data.bus_location=bus_location;
				$.ajax({
					type: "POST",
					url:"<?php echo site_url('reports/standard_OppAnalysisController/fetch_filter_data')?>",
					dataType:'json',
					...
					success: function(data){
                        ...
                        $.each(data , function(i, val){
                               if(i=='business_location')
                               {
                                  filter='bus_location';
                               }else if(i=='clientele_industry'){
                                  filter='industry';
                               }else{
                                  filter=i;
                               }
        						...
        						for(var x=0;x<val.length; x++){
        							options += "<option value='"+val[x].dataid+"'>"+ val[x].dataname+"</option>";
			...
			function generateReport(){
				...
				obj.bus_location = $("#bus_location").val();
				...
				obj.type='opp_velocity_analysis';
				obj.subtype='Summary';
			...
			function buildHtmlTable(myList,selector, inputdata, state) {
				...
				if(state == "Saved"){
					$("#report_title").html(state+" report  for <u>"+ inputdata.name +"</u>  on <u>"+ $("#start_date input").val() /*+"</u> to <u>"+ $("#end_date input").val() +"</u><hr>"*/);
				}else{
					$("#report_title").html(state+" report  for <u>"+ inputdata.name +"</u>  on <u>"+ moment(inputdata.startDate, "DD-MM-YYYY").format('DD-MM-YYYY') /*+"</u> to <u>"+ moment(inputdata.endDate, "DD-MM-YYYY").format('DD-MM-YYYY') +"</u><hr>"*/);
				}
				...
				<div class="page-container">
					<div class="row report_input_data" >
						...
						<div class="col-md-2">
							...
							<select class="form-control" id="product" onchange="list_view('industry')">
							</select>
							<span class="error-alert"></span>
						</div>
                        <div class="col-md-2">
							<label for="industry">Industry</label>
							<select class="form-control" id="industry" onchange="list_view('bus_location')">
							</select>
							<span class="error-alert"></span>
						</div>
                        <div class="col-md-2">
							<label for="bus_location">Business Location</label>
							<select class="form-control" id="bus_location" onchange="list_view('sell_type')" >
							</select>
							<span class="error-alert"></span>
						</div>
                        <div class="col-md-2">
							<label for="sell_type">Sell Type</label>
							<select class="form-control" id="sell_type" >
							</select>
							<span class="error-alert"></span>
						</div>
							<div class="col-md-3">
