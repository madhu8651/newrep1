...
<script>
	...
	$(document).ready(function(){
		...
		/*
		not required for new requirement (23-08-2018)
	...
	function init_stageview_page() {
		...
		$.ajax({
			...
			success : function(data){
                ...
                var count=1;
                $.each(data.docrowcnt, function(key, value) {
                  document_path = value.document_path.split('/');
                  //console.log(count++ +"--"+ document_path[(document_path.length-1)])
                  $('#doc_upload_remarks').closest('div').append('<div>'+ count++ + ') ' + document_path[(document_path.length-1)]+'</div>');
                })
                ...
                //leadclosebit
				//close_progress_btn
                //Close won is enabled only if current stage has closed won attribute is set in admin module
                ...
                if(data.leadclosebit == 0){
				  // $('#close_lead_cust input[type=checkbox]').attr('disabled', 'disabled');
				   $('#close_lead_cust').hide();
                  //  $('#close_lead_cust').append('<div class="col-md-12 text-center infoMsg" style="color:red"><p>'+data.leadclosereason+'</p></div>');
				}else{
				   $('#close_lead_cust input[type=checkbox]').removeAttr('disabled');
				}
		...
		for(i=0;i<data.length;i++){
			...
			if(data[i].docs.length>0){
				...
				for(j=0;j<data[i].docs.length;j++){
					...
					'<li>'+
					'<a href="<?php echo site_url();?>'+path+'" target="_blank">';
	...
	function save_products() {
		...
		if($.trim($("#mul_prod_remarks").val()) == ""){
			...
			return;
		} if(!comment_validation($.trim($("#mul_prod_remarks").val()))){
				...
				if(history) {
					...
					for (var i = 0; i < history.length; i++) {
						if (mapping_ids.indexOf(history[i].mapping_id) < 0) {
							...
							var accept_actions = ['stage accepted', 'ownership accepted','reopen'];
	...
	function setup_questionnaire(data,typeofsheet,qual_status) {
		if(data == false && typeofsheet == 'quaans'){
	...
	function validate_attributes(){
		...
		if ((stage_owner == '-') || (stage_owner == null)) {
            ...
            $('#alert').modal('show');
			$("#alert .modal-body center span").text("Can't progress without a stage owner");
			return isValid;
		}
		if ((owner == '-') || (owner == null)) {
	...
	function opp_save() {
		...
		if ((stage_owner == '-') || (stage_owner == null)) {
			//alert("Can't progress without a stage owner");
            $('#alert').modal('show');
			$("#alert .modal-body center span").text("Can't progress without an stage owner");
			return ;
		}
		...
		if ((owner == '-') || (owner == null)) {
			//alert("Can't progress without an opportunity owner");
            $('#alert').modal('show');
			$("#alert .modal-body center span").text("Can't progress without an opportunity owner");
			return ;
		}
		...
		$.ajax({
			...
			success : function(data){
				...
				if (data.status) {
					//alert('Data is saved.');
                    $('#alert').modal('show');
				    $("#alert .modal-body center span").text("Data is saved.");
					cancel1();
					init_stageview_page();
				}
			},
			...
			success : function(data){
				...
				if (fileCheck == true) {
					...
					if ((fileInput.files==null || fileInput.files.length <= 0)) {
						...
						setup_questionnaire(data,'quaans','no');
					}
				} else {
					setup_questionnaire(data,'quaans','no');
				}
		...
		if(totalQuestions != selectedQuestions){
			...
			$.ajax({
				...
				success: function (data) {
					...
					if (data == 0) {
						//alert("Attempted Qualifier was unsuccessful. Opportunity cannot be progressed to the next stage.");
                        $('#alert').modal('show');
				        $("#alert .modal-body center span").text("Attempted Qualifier was unsuccessful. Opportunity cannot be progressed to the next stage.");
					} else if (data == 1) {
						if (next_stage_seq_no() == "100") {
							close_opportunity();
						} else {
	...
	function opp_progress_final(){
		...
		$.ajax({
			...
			success : function(data){
				...
				if (data.status) {
					cancel1();
					//alert('Congratulations!\nYour opportunity has progressed to the next stage successfully!'+ data.stgownby);
                    $("body").append('<div class="mask custom-alert"></div><div class="alert alert-danger row custom-alert"> <div class="col-md-9">Congratulations!\nYour opportunity has progressed to the next stage successfully!'+ data.stgownby+'</div><div class="col-md-3"><span class="btn Ok">Ok</span>&nbsp;&nbsp;<span class="btn notOk">Cancel</span></div></div>');
                	$(".Ok").click(function(){
                	    $(".custom-alert").remove();
                		loaderShow();
					    window.location.reload();
	...
	function reject_stage()	{
		...
		$.ajax({
			...
			success : function(data){
				...
				if (data.status == true) {
					cancel1();
					//alert('Your opportunity has been rejected to the predefined stage successfully.');
                    $("body").append('<div class="mask custom-alert"></div><div class="alert alert-danger row custom-alert"> <div class="col-md-9">Your opportunity has been rejected to the predefined stage successfully.</div><div class="col-md-3"><span class="btn Ok">Ok</span>&nbsp;&nbsp;<span class="btn notOk">Cancel</span></div></div>');
                	$(".Ok").click(function(){
                	    $(".custom-alert").remove();
                		loaderShow();
					    window.location.reload();
		...
		$.ajax({
			...
			success : function(data){
				...
				if (data.status) {
                    ...
                    $("body").append('<div class="mask custom-alert"></div><div class="alert alert-danger row custom-alert"> <div class="col-md-9">Opportunity Closed!</div><div class="col-md-3"><span class="btn Ok">Ok</span>&nbsp;&nbsp;<span class="btn notOk">Cancel</span></div></div>');
                	$(".Ok").click(function(){
                	    $(".custom-alert").remove();
                	    if ($('#close_won').prop('checked')== true) {
      						cancel1();
      						show_confetti();
      						setTimeout(function() {
      							hide_confetti();
			...
			success : function(data){
			   ...
			   /*	if (data.errors.length > 0) {
			    ...
			    if(data.status)
                {
                   if(data.status1)
                   {
                      alert("Opportunity and associated Lead Closed");
                      cancel1();
    				   loaderShow();
    				   window.location.reload();
                   }else{
...
<div class="content-wrapper body-content">
	<div class="col-lg-12 column" id="content">
		...
		<div class="row static " style="text-align:center">
			...
			<input type="button" class="btn" href="#updatePopup" onclick="progress_popup();" value="Progress" id="opp_progress_btn">
            <input type="button" class="btn" href="#closeOpportunityPopup"  value="Close Opportunity" onclick="closeOpportunity_popup()" id="opp_close_btn">
		</div>
	</div>
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row stage_popup">
							<!--<a href="<?php echo site_url('sales_mytaskController')?>"> Log an Activity</a>  -->
					</div>
	...
	<div id="opp_taskdetails" class="modal fade" data-backdrop="static" data-keyboard="false">
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-body">
					<div class="row stage_popup">
							<a href="<?php echo site_url('sales_calendarController')?>"> Schedule a New Activity</a>
					</div>
