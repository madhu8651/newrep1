...
.body-content .nav.nav-tabs .active a{
   ...
   <script>
	...
	function loadpage(){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sales_stageController/get_data'); ?>",
			dataType : 'json',
			cache : false,
			success : function(data){
                ...
                if(data.length==0 || data[0].hasOwnProperty("CYCLE_ID")==false){
					...
					$("#no_data_section").removeClass('none').append("<center style='padding-top: 60px;'><h4>No data available. Visit <b>Sales Cycle</b> page to add data<br> <a href='<?php echo site_url('admin_sales_cycleController'); ?>'><u>Sales Cycle </u></a></h4></center>");
                }else{
					var row = "";
...
function renderSingleTable1(cycleid,oppchk,dupbit){
        ...
        addObj.cycleid=cycleid;
         ...
         $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sales_stageController/get_data2'); ?>",
			dataType : 'json',
			...
			success : function(data){
                    ...
                    if(oppchk == "1"){
                        ...
                        i=0;
            						...
            						row1 += "<tr><th width='10%'>Sl No</th><th width='25%'>Stage Name</th><th width='30%'>Description</th><th width='25%'>Master Stage</th><th width='10%'></th></tr></thead><tbody >";
            						var duplicateBtn = 0;
                        ...
                        if(data[i].hasOwnProperty("stagedata")){
						row1 += '<table class="table"><thead>';
						row1 += "<tr><th width='10%'>Sl No</th><th width='25%'>Stage Name</th><th width='30%'>Description</th><th width='25%'>Master Stage</th><th width='10%'>Edit</th></tr></thead><tbody class='ui-sortable'>";
							var duplicateBtn = 0;
        						for(var k=0; k < data[i].stagedata.length; k++ ){
        							var jsondata=JSON.stringify([{"CYCLE_ID":data[i].CYCLE_ID,
        														"stage_name":data[i].stagedata[k].stage_name,
    ...
    /* --------------------------------------------------------------------------------------------------------------- */
        ...
        function createDuplicate(cycleId){
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('admin_sales_stageController/get_cycle'); ?>",
				dataType : 'json',
				cache : false,
				success : function(data){
					if(error_handler(data)){
        ...
        function duplicateSubmit(){
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('admin_sales_stageController/create_duplicate'); ?>",
				data : JSON.stringify(addObj),
				dataType : 'json',
				cache : false,
				success : function(data){
        ...
        function compose(){
            ...
            $.ajax({
                      type : "POST",
                      url : "<?php echo site_url('admin_sales_stageController/get_master_stage'); ?>",
                      ...
                      success : function(data){
						  if(error_handler(data)){
                            ...
                            if(str=="null"){
                                    $('#addmodal').modal('hide');
                                    $('#alert').modal('show');
						            $("#alert .modal-body center span").text("Master Stage are not assigned to existing stages");
                            }else{
                                $('#addmodal').modal('show');
 ...
 /* -------------------------------------- save stage code  ----------------------------------------------------------------------- */
    function addSave(){
            ...
            if($.trim($("#add_masterstage").val())==""){
				$("#add_masterstage").closest("div").find("span").text("Master Stage is required.");
	...
	function proceed1(){
			...
			$.ajax({
				    type : "POST",
					url : "<?php echo site_url('admin_sales_stageController/post_data'); ?>",
					dataType : 'json',
					data : addSalesStage,
					cache : false,
					success : function(data){
 ...
 /* ----------------------------------- on select of each table row fill cycle, stage and description */
    function selrow(obj){
            ...
            if(obj[0].master_stageid== null){
                    ...
                    $.ajax({
                              type : "POST",
                              url : "<?php echo site_url('admin_sales_stageController/get_master_stage_edit'); ?>",
                              data : JSON.stringify(addObj),
                              dataType : 'json',
                              cache : false,
                              success : function(data){
 ...
 /* ---------------------------------------------------------------------------------------------------------------------------- */
    function editsave(){
            ...
            if($.trim($("#edit_masterstage").val())==""){
				$("#edit_masterstage").closest("div").find("span").text("Master Stage is required.");
				return;
			}else{
    ...
    function proceed(){
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('admin_sales_stageController/update_data'); ?>",
				dataType : 'json',
				data : stageobj,
				cache : false,
				success : function(data){
 ...
 /* ------------------------------------------------- save the row order of stages ----------------------------------------------------------------- */
    function saveorder(cycleid){
            ...
            $.ajax({
    				    type : "POST",
    					url : "<?php echo site_url('admin_sales_stageController/update_roworder'); ?>",
    					dataType : 'json',
    					data : JSON.stringify(addObj),
    					cache : false,
    					success : function(data){
	...
	<body class="hold-transition skin-blue sidebar-mini">
		...
		<div class="content-wrapper body-content">
				...
				<div class="row header1">
					<div class="col-xs-2 col-sm-2 col-md-4 col-lg-4 aa">
						<span class="info-icon">
							<div >
								<img src="<?php echo base_url(); ?>images/new/i_off.png" onmouseover="this.src='<?php echo base_url(); ?>images/new/i_ON.png'" onmouseout="this.src='<?php echo base_url(); ?>images/new/i_off.png'" alt="info" width="30" height="30"  data-toggle="tooltip" data-placement="right" title="Sales Stage Flowchart"/>
							</div>
						</span>
			...
			<ul class="nav nav-tabs">
				<li>
					<a href="<?php echo site_url('admin_sales_cycleController'); ?>">Sales Cycle</a>
				</li>
				<li class="active">
					<a href="<?php echo site_url('admin_sales_stageController'); ?>">Sales Stage Flowchart</a>
				</li>
			</ul>
					...
					<div class="modal-content">
						<form id="editpopup" class="form" action="#" method="post" name="adminClient">
							<div class="modal-header">
								 ...
								 <h4 class="modal-title">Edit Sales Stages</h4>
							</div>
                            <div class="modal-body">
								...
								<div class="row">
									<div class="col-md-3 col-sm-3 col-xs-3">
										<label for="edit_masterstage">Master Stage*</label>
									</div>
									<div class="col-md-9 col-sm-9 col-xs-9">
				...
				<div class="modal-dialog">
					<div class="modal-content">
						<form id="addpopup" class="form" action="#" method="post" name="adminClient">
							<div class="modal-header">
								 <span class="close" onclick="cancel()">x</span>
								 <h4 class="modal-title">Add Sales Stages</h4>
							</div>
							<div class="modal-body">
								...
								<div class="row">
									<div class="col-md-3 col-sm-3 col-xs-3">
										<label for="add_masterstage">Master Stage*</label>
									</div>
									<div class="col-md-9 col-sm-9 col-xs-9">
