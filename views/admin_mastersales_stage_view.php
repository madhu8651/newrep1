...
<html lang="en">
   ...
   <script>
	...
	function loadpage(){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_mastersales_stageController/get_data'); ?>",
			dataType : 'json',
			cache : false,
 ...
 function renderSingleTable1(cycleid,dupbit){
        ...
        addObj.cycleid=cycleid;
         ...
         $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_mastersales_stageController/get_data2'); ?>",
			dataType : 'json',
 ...
 /*--------------------------------------- auto fill cycle select box --------------------------------------------------------  */
    function auto_load(){
	   $.ajax({
		type : "POST",
		url :"<?php echo site_url('admin_mastersales_stageController/get_cycle_name'); ?>" ,
		dataType : 'json',
		cache : false,
		success : function(data){
    			if(error_handler(data)){
 ...
 /* -------------------------------------- save stage code  ----------------------------------------------------------------------- */
	...
	function proceed1(){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_mastersales_stageController/post_data'); ?>",
			dataType : 'json',
			data : addSalesStage,
			cache : false,
			success : function(data){
 ...
 /* ---------------------------------------------------------------------------------------------------------------------------- */
    ...
    function proceed(){
			...
			$.ajax({
				    type : "POST",
					url : "<?php echo site_url('admin_mastersales_stageController/update_data'); ?>",
					dataType : 'json',
					data : stageobj,
					cache : false,
					success : function(data){
 ...
 /* ------------------------------------------------- save the row order of stages ----------------------------------------------------------------- */
    function saveorder(cycleid){
		  ...
		  loaderShow();
            $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_mastersales_stageController/update_roworder'); ?>",
			dataType : 'json',
			data : JSON.stringify(addObj),
			cache : false,
			success : function(data){
     ...
     /* --------------------------------------------------------------------------------------------------------------- */
        ...
        function createDuplicate(cycleId){
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('admin_mastersales_stageController/get_cycle'); ?>",
				dataType : 'json',
				cache : false,
				success : function(data){
					if(error_handler(data)){
        ...
        function duplicateSubmit(){
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('admin_mastersales_stageController/create_duplicate'); ?>",
				data : JSON.stringify(addObj),
				dataType : 'json',
				cache : false,
				success : function(data){
	...
	<body class="hold-transition skin-blue sidebar-mini">
		...
		<div class="content-wrapper body-content">
				...
				<div class="row header1">
					<div class="col-xs-2 col-sm-2 col-md-4 col-lg-4 aa">
						<span class="info-icon">
							<div >
								<img src="<?php echo base_url(); ?>images/new/i_off.png" onmouseover="this.src='<?php echo base_url(); ?>images/new/i_ON.png'" onmouseout="this.src='<?php echo base_url(); ?>images/new/i_off.png'" alt="info" width="30" height="30"  data-toggle="tooltip" data-placement="right" title="Master Stage Flowchart"/>
							</div>
						</span>
			...
			<ul class="nav nav-tabs">
				<li>
					<a href="<?php echo site_url('admin_mastersales_cycleController'); ?>">Master Sales Cycle</a>
				</li>
				<li class="active">
					<a href="<?php echo site_url('admin_mastersales_stageController'); ?>">Master Stage Flowchart</a>
				</li>
			</ul>
					...
					<div class="modal-content">
						<form id="editpopup" class="form" action="#" method="post" name="adminClient">
							<div class="modal-header">
								 ...
								 <h4 class="modal-title">Edit Master Stages</h4>
							</div>
				...
				<div class="modal-dialog">
					<div class="modal-content">
						<form id="addpopup" class="form" action="#" method="post" name="adminClient">
							<div class="modal-header">
								 <span class="close" onclick="cancel()">x</span>
								 <h4 class="modal-title">Add Master Stages</h4>
							</div>
							<div class="modal-body">
                    ...
                    <div class="modal-content">
                        <form id="editpopup" class="form" action="#" method="post" name="adminClient">
							...
							<div class="modal-body">
								<div class="row">
								  <div class="col-md-3">
									  <label for="edit_cycle">Master Sales Cycle*</label>
								  </div>
								  <div class="col-md-9">
