...
/* ------------------------------------------------------------ on load fill the table -------------------------------------------------------------------- */
	function loadpage(){
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_sales_stage_flowchartController/get_data'); ?>",
			dataType : 'json',
			cache : false,
...
function renderSingleTable(cycleid){
        ...
        addObj.cycleid=cycleid;
         ...
         $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_sales_stage_flowchartController/get_data1'); ?>",
			dataType : 'json',
...
function get_allocation(cycleid){
     ...
     $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_sales_stage_flowchartController/get_owner'); ?>",
			dataType : 'json',
            data: JSON.stringify(addObj),
			cache : false,
			success : function(data){
...
function formcustomfield(){
     $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_sales_stage_flowchartController/get_customfield'); ?>",
			dataType : 'json',
			cache : false,
			success : function(data){
				if(error_handler(data)){
                ...
                for(var i=0;i<data.length;i++){
                        var attr_id = data[i].support_attribute_name;
                        var strLower = attr_id.toLowerCase();
                        var strLower2 = data[i].support_attribute_id;
                        strLower1=strLower.replace(/\s/g, '');

                        custom_row += "<tr> <td> <label for=\""+strLower2+"\"> <input onchange='renderSavedValues(\""+strLower2+"\")' type='checkbox' name=\""+strLower2+"\" value=\""+strLower2+"\" id=\""+strLower2+"\" class='custom_checkbox_class'/> "+attr_id+" </label> </td> <td> <input class='form-control' disabled type='text'  id=\"attr_"+strLower2+"\" placeholder='Reference Text' size='45' /> <input class='form-control' type='hidden'  id=\"hid_attr_"+strLower1+"\" placeholder='Reference Text' size='45'> <span class='error-alert'></span> </td> </tr>";
                }
                $('#custom_fields').append(custom_row);
			}
         });
...
/* ------------------------------------------------------- function to fill qualifier saved and not saved one ----------------------------------------------------------------------- */
    function loadqual(qualval,cycleid){
         ...
         $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_sales_stage_flowchartController/get_qualifier'); ?>",
			dataType : 'json',
            data: JSON.stringify(addObj),
            cache : false,
            success : function(data){
...
/* ------------------------------------------------------- function to fill qualifier which are not saved  ----------------------------------------------------------------------- */
    ...
    function loadqual1(cycleid){
         ...
         $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_sales_stage_flowchartController/get_qualifier1'); ?>",
			dataType : 'json',
            data: JSON.stringify(addObj),
            cache : false,
            success : function(data){
...
/* ------------------------------------------------------- function to fill action stages above the selected stage ----------------------------------------------------------------------- */
  function load_actnstage(stage_seq,cycleid,stageval){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_sales_stage_flowchartController/get_actnstage'); ?>",
			dataType : 'json',
			data: JSON.stringify(addObj),
			cache : false,
			success : function(data){
...
/* ----------------------------------------- on click of pencil icon fill the pop up ---------------------------------------------------------------------- */
 ...
 function saveName(stage_id, CYCLE_ID){
	 ...
	 $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_sales_stage_flowchartController/post_desc'); ?>",
			dataType : 'json',
...
/* //----------------------------------------------Edit Sales stage----------------- -----------------------------------------------------------------------*/
		function editsave(){
			...
			if(succ==0){
			    ...
			    $.ajax({
        				type : "POST",
        				url : "<?php echo site_url('admin_sup_sales_stage_flowchartController/post_data'); ?>",
        				dataType : 'json',
                        data: JSON.stringify(addObj),
        				cache : false,
        				success : function(data){
				...
				<div class="row header1">
					<div class="col-xs-2 col-sm-2 col-md-4 col-lg-4 aa">
						<span class="info-icon">
							<div>
								<img src="<?php echo base_url(); ?>images/new/i_off.png" onmouseover="this.src='<?php echo base_url(); ?>images/new/i_ON.png'" onmouseout="this.src='<?php echo base_url(); ?>images/new/i_off.png'" alt="info" width="30" height="30"  data-toggle="tooltip" data-placement="right" title="Support Stage Attributes"/>
							</div>
						</span>
					...
					<div class="col-xs-8 col-sm-8 col-md-4 col-lg-4 pageHeader1 aa">
						<h2>Support Stage Attributes</h2>
					</div>
					<div  class="col-xs-2 col-sm-2 col-md-4 col-lg-4 aa">
					</div>
					...
					<div class="modal-content">
						<form id="stageAttributeFrm" class="form" name="adminClient">
							<div class="modal-header">
								...
								<h4 class="modal-title">Suport Stage Attributes For <span id="stage_name_t"></span></h4>
							</div>
								...
								<table class="table" id="workingTable">
									...
									<tbody>
                                        ...
                                        <tr>
											<td>
												<label for="agreedtat">
													<input onchange="renderSavedValues('agreedtat')" type="checkbox" name="approve4" value="agreedtat" id="agreedtat" class="custom_checkbox_class" /> Agreed TAT
												</label>
											</td>
											<td>
												<input class="form-control default-disabled" type="text"  id="attr_agreedtat" placeholder="Reference Text" size="45">
												<input class="form-control" type="hidden"  id="hid_attr_agreedtat" placeholder="Reference Text" size="45">
												<span class="error-alert"></span>
											</td>
										...
										<tr>
											<td>
												<label for="custcode">
													<input onchange="renderSavedValues('custcode')" type="checkbox" name="approve6" value="custcode" id="custcode" class="custom_checkbox_class" /> Customer Code
												</label>
											</td>
											<td>
												<input class="form-control default-disabled" type="text"  id="attr_custcode" placeholder="Reference Text" size="45">
												<input class="form-control" type="hidden"  id="hid_attr_custcode" placeholder="Reference Text" size="45">
												<span class="error-alert"></span>
											</td>
                                        ...
                                        <tr>
											<td>
												<label for="updatecustemail">
													<input onchange="renderSavedValues('updatecustemail')" type="checkbox" name="approve8" value="updatecustemail" id="updatecustemail" class="custom_checkbox_class" /> Update Customer Email
												</label>
											</td>
											<td>
												<input class="form-control default-disabled" type="text"  id="attr_updatecustemail" placeholder="Reference Text" size="45">
												<input class="form-control" type="hidden"  id="hid_attr_updatecustemail" placeholder="Reference Text" size="45">
												<span class="error-alert"></span>
											</td>
										</tr>
