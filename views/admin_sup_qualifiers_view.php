...
@-moz-document url-prefix() {
 ...
 function loadpage(){
    ...
    $.ajax({
    	type : "POST",
    	url : "<?php echo site_url('admin_sup_qualifiersController/get_qualifier'); ?>",
    	dataType : 'json',
    	cache : false,
	...
	function saveName(name , rowId){
		...
		$('[data-original-title]').popover('hide');
		 $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_qualifiersController/update_qualifiername'); ?>",
			dataType : 'json',
   ...
   function loadques(qualid){
        ...
        $.ajax({
                 type : "POST",
                 url : "<?php echo site_url('admin_sup_qualifiersController/get_queansdata') ?>",
                 dataType : 'json',
                 data : JSON.stringify(addObj),
                 cache : false,
                 success : function(data){
...
function answerSaveFunc(){
	...
	$.ajax({
		type : "POST",
		url : "<?php echo site_url('admin_sup_qualifiersController/post_queans'); ?>",
...
function saveQ(id){
	...
	if(succ==0){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_sup_qualifiersController/update_queans'); ?>",
			dataType : 'json',
...
$("#que"+id).html(questionreset);
   ...
   function delrow(obj){
            ...
            $.ajax({
				type : "POST",
				url : "<?php echo site_url('admin_sup_qualifiersController/delete_question'); ?>",
				data : JSON.stringify(addObj),
				cache : false,
				success : function(data1){
					if(error_handler(data1)){
        ...
        /* --------------------------------- update mandatory check ---------------------------------------------- */
            function updatechk(id,question_id){
                ...
                $.ajax({
    				type : "POST",
    				url : "<?php echo site_url('admin_sup_qualifiersController/update_mandatorychk'); ?>",
    				data : JSON.stringify(addObj),
    				cache : false,
    				success : function(data1){
						if(error_handler(data1)){
		...
		function saveOrder() {
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('admin_sup_qualifiersController/update_roworder'); ?>",
				data : JSON.stringify(addObj),
				cache : false,
				success : function(data1){
							if(error_handler(data1)){
       ...
       /* -------------------------------------------------------------------------------------------------------------------------------------- */
		...
		function addsave(){
		   $("#addsave1").prop('disabled', true);
				...
				$.ajax({
					type : "POST",
					url : "<?php echo site_url('admin_sup_qualifiersController/post_data'); ?>",
					dataType : 'json',
					data : JSON.stringify(addObj),
					cache : false,
					success : function(data){
				...
				<div class="row header1">
					<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 aa">
						<span class="info-icon">
							<div >
								<img src="<?php echo base_url(); ?>images/new/i_off.png" onmouseover="this.src='<?php echo base_url(); ?>images/new/i_ON.png'" onmouseout="this.src='<?php echo base_url(); ?>images/new/i_off.png'" alt="info" width="30" height="30"  data-toggle="tooltip" data-placement="right" title="Support Qualifier list"/>
							</div>
						</span>
					</div>
					<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 pageHeader1 aa">
							<h2>Support Qualifier list</h2>
					</div>
					<div  class="col-xs-4 col-sm-4 col-md-4 col-lg-4  aa">
						 <div class="addBtns">
								...
								<div id="mySidenav" class="col-md-6">
									...
									<div class="row que-ans-sec">
										<div class="col-md-3" style="padding-top: 6px;">
											<a href="#" id="addAnswer" class="ad_ans">
