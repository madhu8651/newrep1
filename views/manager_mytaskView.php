...
<html lang="en">
	...
	#start_date[readonly],
	#cmp_start_date[readonly],
	#start_date2[readonly]
	{
...
#mail_body{
	...
	<script>
	 var navigatorChk,team_mid, team_data, ticket_val, completed_value;
	 var doc_data = "";
	<?php  
	...
	function team_members_Add(){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('manager_mytaskController/userList'); ?>",
			dataType : 'json',
			cache : false,
	...
	$(document).ready(function(){
		...
		$("#complete_actve_duration2").datetimepicker({
			ignoreReadonly:true,
			allowInputToggle:true,
			format:'HH:mm',
			defaultDate:'1970-01-01 00:00'
		...
		var data_opp='';
		/* function team_mem(){
			var val = $("#team_members1").val();
			$("#contact").closest(".row").hide();
			$("#lead, #customer, #opportunity, #internal").val("");
			$(".lead_row, .cust_row, .opp_row, .inter_row").hide();
			$("#task_type").val("");
			if(val){
		...
		function select_ticket_type(){
			...
			$.ajax({ 	/* //ajax call for get rep_names */
				type : "POST",
				url : "<?php echo site_url('manager_mytaskController/get_contactsForLead'); ?>",
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('manager_mytaskController/get_team_members'); ?>",
				data:JSON.stringify(leadObj),
				dataType : 'json',					
				cache : false,
				success : function(data)	{
					...
					var select1 = $("#team_members1"), options1 = '<option value="">Choose Team members</option>';
					var select2 = $("#team_members"), options2 = "<option value=''>Choose Team members</option>";
					select1.empty();
					select2.empty();
					for(var i=0;i<data.length; i++)	{					
						if(data[i].user_name == "Me"){
							ownid = data[i].user_id;
						}
						options1 += "<option value='"+data[i].user_id+"'>"+ data[i].user_name +"</option>";
						options2 += "<option value='"+data[i].user_id+"'>"+ data[i].user_name +"</option>";
					}
		...
		function save_id(val){
			...
			$.ajax({ 
				type : "POST",
				url : "<?php echo site_url('manager_mytaskController/get_leads'); ?>",
				dataType : 'json',		
				...
				success : function(data)	{
					...
					$('#internal').on('typeahead:selected', function (e, datum) {
						...
						select.append(options);
						var leadObj = {};
						leadObj.leadid = datum.user_id;
						leadObj.type='internal';
						$.ajax({
								type : "POST",
								url : "<?php echo site_url('manager_mytaskController/get_team_members'); ?>",
								data:JSON.stringify(leadObj),
					...
					$('#lead').on('typeahead:selected', function (e, datum) {
						...
						if ($("#lead").length <= 1) {
							...
							if($('.ticket_main').css('display') != 'none'){
								...
								$.ajax({ 
									type : "POST",
									url : "<?php echo site_url('manager_mytaskController/get_support_request'); ?>",
									dataType : 'json',		
									data : JSON.stringify(leadObj),
									cache : false,
									success : function(data){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('manager_mytaskController/get_opportunities'); ?>",
			dataType : 'json',
			data:JSON.stringify(teamObj),	
			cache : false,
			success : function(data){
	...
	function get_opp(){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('manager_mytaskController/get_contactsForOpportunity'); ?>",
			dataType : 'json',
	...
	function pageload(a){
		...
		$.ajax({
			type : "POST",
			url :"<?php echo site_url('manager_mytaskController/checkForPending');?>",
			dataType:'json',
			cache : false,
			success :function(data){
				$('#rederr').hide(); 
		...
		/* ----------------------------------------------------------------------------------------------
		**************************** Open event list fetching *****************************************
		-------------------------------------------------------------------------------------------------- */
				$.ajax({
					type : "POST",
					url : "<?php echo site_url('manager_mytaskController/get_mytask'); ?>",
					dataType : 'json',
					cache : false,
					success : function(data){
						loaderHide();
						...
						for(i=0; i < data.length; i++ ){
							...
							if(data[i].status=='pending'){
								...
								var status_name = "";
								if(data[i].reminderstatus == "pending"){
									status_name = "Pending";
								}
								if(data[i].reminderstatus == "scheduled"){
								...
								if(ownid == data[i].user_id){
									row += "<tr style='font-weight: bold;'><td>" + (i+1)+ "</td><td>" + data[i].event_name+ "</td><td>" + givenDate.format('lll') +"</td><td><b>" + data[i].lookup_value + "</b></td><td>" + type + "</td><td>" + data[i].employeename + "</td><td>"+ownerName+" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td style='color:blue'>"+data[i].status+"</td><td><a data-toggle='modal' href='#completed' onclick='selrow(\""+compositKey+"\")'><button type='button' class='btn completed_btn'>update</button></a></td><td><a onclick='reschedulepopup(\""+compositKey+"\")' class='glyphicon glyphicon-time'  title='Reschedule'></a></td></tr>";
								}else{
									row += "<tr><td>" + (i+1)+ "</td><td>" + data[i].event_name+ "</td><td>" + givenDate.format('lll') +"</td><td><b>" + data[i].lookup_value + "</b></td><td>" + type + "</td><td>" + data[i].employeename + "</td><td>"+ownerName+" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td style='color:blue'>"+data[i].status+"</td><td><a data-toggle='modal' href='#completed' onclick='selrow(\""+compositKey+"\")'><button type='button' class='btn completed_btn'>update</button></a></td><td><a onclick='reschedulepopup(\""+compositKey+"\")' class='glyphicon glyphicon-time'  title='Reschedule'></a></td></tr>";
								}
		...
		/* ----------------------------------------------------------------------------------------------
		**************************** closed event list fetching -- End*****************************************
		-------------------------------------------------------------------------------------------------- */
				$.ajax({
					type : "POST",
					url : "<?php echo site_url('manager_mytaskController/get_activity'); ?>",
					dataType : 'json',					
					cache : false,
					success : function(data)	{
	...
	-------------------------------------------------------------------------------------------------- */
	function completed_data(a){
		/* ----------------------------------------------------------------------------------------------
	**************************** closed event list fetching *****************************************
	-------------------------------------------------------------------------------------------------- */
			...
			$.ajax({
					type : "POST",
					url : "<?php echo site_url('manager_mytaskController/getCompletedReshceduleTask'); ?>",
					dataType : 'json',
					cache : false,
					success : function(data){
						loaderHide();
						...
						$('.closeinput').val('');
						$('#tableBody1').empty();
						var row = "", employe_name = "", scheduleType = '', titleName = '';
						var ownerName="", state = "closed";
						...
						for(i=0; i < data.length; i++ ){
							...
							//----------
							/* employe_name = data[i].employeename;
							...
							if(data[i].reminderstatus == 'reschedule'){
								if(ownid == data[i].user_id){
									row = "<tr class='warning' style='font-weight: bold;'><td>" + (i+1)+ "</td><td>" + titleName+ "</td><td> "+ closedDate.format('lll') +"</td><td> " + scheduleType +"</td><td>" + data[i].lookup_value + "</td><td>" + Prospect+ type + "</td><td>" + employe_name + "</td><td>"+ ownerName +" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td>Rescheduled</td><td><a onclick='reschedulepopup1(\""+compositKey+"\",\""+state+"\")' class='glyphicon glyphicon-eye-open'></a></td></tr>";
								}else{
									row = "<tr class='warning'><td>" + (i+1)+ "</td><td>" + titleName+ "</td><td> "+ closedDate.format('lll') +"</td><td> " + scheduleType +"</td><td>" + data[i].lookup_value + "</td><td>" + Prospect+ type + "</td><td>" + employe_name + "</td><td>"+ ownerName +" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td>Rescheduled</td><td><a onclick='reschedulepopup1(\""+compositKey+"\",\""+state+"\")' class='glyphicon glyphicon-eye-open'></a></td></tr>";
								}
							}else if(data[i].reminderstatus == 'cancel'){
								if(ownid == data[i].user_id){
									row = "<tr style='font-weight:bold'><td>" + (i+1)+ "</td><td>" + titleName+ "</td><td> "+ closedDate.format('lll') +"</td><td> " + scheduleType +"</td><td>" + data[i].lookup_value + "</td><td>" + Prospect+ type + "</td><td>" + employe_name + "</td><td>"+ ownerName +" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td>Cancelled</td><td><a onclick='reschedulepopup1(\""+compositKey+"\",\""+state+"\")' class='glyphicon glyphicon-eye-open'></a></td></tr>";
								}else{
									row = "<tr><td>" + (i+1)+ "</td><td>" + titleName+ "</td><td> "+ closedDate.format('lll') +"</td><td> " + scheduleType +"</td><td>" + data[i].lookup_value + "</td><td>" + Prospect+ type + "</td><td>" + employe_name + "</td><td>"+ ownerName +" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td>Cancelled</td><td><a onclick='reschedulepopup1(\""+compositKey+"\",\""+state+"\")' class='glyphicon glyphicon-eye-open'></a></td></tr>";
								}
							}
								...
								if(ownid == data[i].user_id){
									row = "<tr style='font-weight: bold;'><td>" + (i+1)+ "</td><td>" + titleName+ "</td><td>" +closedDate.format('lll')+"</td><td>" +scheduleType +"</td><td>" + data[i].lookup_value + "</td><td>" + Prospect+type + "</td><td>" + employe_name + "</td><td>"+ ownerName +" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td>"+data[i].reminderstatus+"</td><td><a onclick='reschedulepopup1(\""+compositKey+"\",\""+state+"\")' class='glyphicon glyphicon-eye-open'></a></td></tr>";
								}else{
									row = "<tr><td>" + (i+1)+ "</td><td>" + titleName+ "</td><td>" +closedDate.format('lll')+"</td><td>" +scheduleType +"</td><td>" + data[i].lookup_value + "</td><td>" + Prospect+type + "</td><td>" + employe_name + "</td><td>"+ ownerName +" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td>"+data[i].reminderstatus+"</td><td><a onclick='reschedulepopup1(\""+compositKey+"\",\""+state+"\")' class='glyphicon glyphicon-eye-open'></a></td></tr>";
								}
								...
								if(ownid == data[i].user_id){
									row = "<tr style='font-weight: bold;'><td>" + (i+1)+ "</td><td>" + titleName+ "</td><td>"+data[i].closed_date +"</td><td>" + scheduleType +"</td><td>" + data[i].lookup_value + "</td><td>" + Prospect + type + "</td><td>" + employe_name + "</td><td>"+ownerName+" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td>"+data[i].reminderstatus+"</td><td><a onclick='reschedulepopup1(\""+compositKey+"\",\""+state+"\")' class='glyphicon glyphicon-eye-open'></a></td></tr>";
								}else{
									row = "<tr><td>" + (i+1)+ "</td><td>" + titleName+ "</td><td>"+data[i].closed_date +"</td><td>" + scheduleType +"</td><td>" + data[i].lookup_value + "</td><td>" + Prospect + type + "</td><td>" + employe_name + "</td><td>"+ownerName+" ("+data[i].Department_name+")"+"</td><td>"+data[i].created_by+"</td><td>"+data[i].reminderstatus+"</td><td><a onclick='reschedulepopup1(\""+compositKey+"\",\""+state+"\")' class='glyphicon glyphicon-eye-open'></a></td></tr>";
								}
						...
						if(data.length <=0 ){
							$('#tableBody1 tr td ').attr("colspan","11");	
						}
						if(a==3){
							/* console.log("colsed a = "+ a) */
							addpopup();
						}
					}
				});	
	...
	function cancel(){	
		$("#completed_starts").prop("disabled", true);
		$("#task_completed").hide();
		$("#task_create").show();
		$("#prev_act, #activity1").show();
		$(".reminder_time_hide").show();
		$("#next_act, #activity").hide();
		$('#actve_duration').data("DateTimePicker").date('00:00')
		$('#timepicker1').hide();
		$('#timepicker').show();
		$(".error-alert").val("");
	...
	function addpopup()	{
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('manager_mytaskController/get_emails'); ?>",
			dataType : 'json',					
			cache : false,
			success : function(data){					
				if(error_handler(data)){
		...
		$("#contact").on('change',function(){
			...
			if(contactObj.contactid !=""){
				$.ajax({ 	/* ajax call for get rep_names */
						type : "POST",
						url : "<?php echo site_url('manager_mytaskController/get_employeeNumbers'); ?>",
	...
	***************************** check event time overlap *************************************
	...
	function overlap(startDateTime,timeDuradion){
			...
			$.ajax({	
		        type : "POST",
		        url : "<?php echo site_url('manager_calendarController/checkduration'); ?>",
						...
						if($("#rederr").length<=0){
							...
							htm = '<div class="mask custom-alert" id="rederr">'+
							   '<div class="alert alert-danger row custom-alert">'+ 
								...
								'</div>'+
						   '</div>';
						   /* $("body").append('<div id="rederr" class="overlap alert alert-danger"><p>One or more event(s) already scheduled for this duration..!</p><p>Do you want to continue?</div>'); */
						   $("body").append(htm);
						}					    	
		        	}else{
						time_overlap = true;
	...
	***************************** save button click --When Completed Task Checkbox is checked *************************************
	...
	function addevent1(a){
        ...
        addEventObj.event_members=		$.trim($("#team_members1").val());
        ...
        addEventObj.event_member_name =	event_member_name;	
		addEventObj.event_activity_name = 	$.trim($("#activity option:selected").text());
		if($(".ticket_main").css("display") != "none"){
		...
		$.ajax({ 	/* //AJAX	call for When task completed button	clicked	 */
			type : "POST",
			url : "<?php echo site_url('manager_mytaskController/add_mytaskcomplete'); ?>",
			dataType : 'json',
	...
	***************************** save button click --When Completed Task Checkbox is Not checked *************************************
	...
	function addevent(a){
		...
		if($("#check_all").prop("checked") == true){
			...
			if($.trim($("#team_members1").val()) == ""){
				$("#team_members1").focus();
				$("#team_members1").closest("div").find("span").text("Team Members is required.");
				return;	
			}
			else{
				$("#team_members1").closest("div").find("span").text("");
		...
		if($("#check_all").prop("checked") == true){
			...
			var event_mem = [];
			var event_mem_name = [];
			event_mem.push($.trim($("#team_members1").val()))
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('manager_mytaskController/add_mytask'); ?>",
			dataType : 'json',
			data : JSON.stringify(addEventObj),					
			cache : false,
			success : function(data){
	...
	***************************** Update button click on main page *************************************
	...
	function selrow(key){
		...
		if(ownid == obj.user_id){			
			...
			}else if(assignedDate <= currentDate){
				row = '<input type="checkbox" id="confirm_process1" onchange="confirm_process(\'edit\')"> <label id="label_confrm" for="confirm_process1"> Task Completed</label>';
			}						
		}
		...
		$("#cmp_lead").val(obj.lead_id);
		//$("#cmp_note").val(window.atob(obj.remarks));
		/* console.log(window.atob(obj.remarks)) */
	...
	function cSave_process(){
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('manager_mytaskController/cancelEvent'); ?>",
			dataType : 'json',
			data : JSON.stringify(addEventObj),					
			cache : false,
			success : function(data){
	...
	function complete_alert(){
		...
		$.ajax({ 	
			...
			success : function(data){
				...
				$("#activity_hidden").val(obj.lookup_value);
				if(obj.type=="lead"){			
					$('#lead2_name1').text("Lead*");
				}else if(obj.type=="customer"){
					$('#lead2_name1').text("Customer*");
				}else if(obj.type=="opportunity"){
					$('#lead2_name1').text("Opportunity*");
				}else if(obj.type=="internal"){
					$('#lead2_name1').text("Internal*");
				}else if(obj.type=="support"){
					$('#lead2_name1').text("Ticket*");
				}
				$('#event_name').val(obj.event_name);
				...
				$.ajax({ 	
					...
					success : function(data){
						...
						$('#complete_email_search').on('typeahead:selected', function (e, datum) {
							...
							/* console.log(email_data.sendSavedID.length) */
	...
	*************************** completed Submitting completed data **********************************
	...
	function processComplete(showAddModel)	{
		...
		addEventObj.completed_start_date = 		moment($.trim($("#completed_starts").val())).format('YYYY-MM-DD');
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('manager_mytaskController/update_mytask'); ?>",
			dataType : 'json',
			data : JSON.stringify(addEventObj),					
			cache : false,
			success : function(data){
	...
	**************************** Reschedule Popup Show with Value *****************************************
	...
	/* ----------------Next-Previous button for view activity only visible for open section -----Starts---------------------- */
		...
		function display(array , j, state){			
			...
			if(array[j].conntype == 'CALL594ce66d07b45' || array[j].conntype == 'CALL594ce66d07b46' || array[j].conntype == 'ME594ce66d07b9fd4'){
				...
				if(array[j].hasOwnProperty('path') && array[j].path != null && array[j].path != 'no_path'){
					html += '<div class="row">'+
						'<div class="col-md-2 col-sm-1 col-xs-1" ></div>'+
						'<div class="col-md-3 col-sm-3 col-xs-3">'+										
							'<label><b>Content:</b></label>'+
						'</div>'+
						'<div class="col-md-5 col-sm-5 col-xs-5">'+
							'<label>'+url_path+array[j].path+'</label>'+
						'</div>'+
					'</div>';
				}else{
					html += '<div class="row">'+
	...
	function reschedulepopup1(key, state){
		...
		$("#event_name3").text(obj.event_name);	
		loaderShow();
		$("#reschedule1 .newView").html('');
		if(state == 'open' || state == "closed"){
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('manager_mytaskController/getAllActivitesOfLeadCustSup'); ?>",
				dataType : 'json',
		...
		data.remmem = obj.reminder_members;	
		if(obj.type=="lead"){			
			$('#lead_name').text("Lead").attr('style','font-weight:bold !important');
		}else if(obj.type=="customer"){
			$('#lead_name').text("Customer").attr('style','font-weight:bold !important');;
		}else if(obj.type=="opportunity"){
			$('#lead_name').text("Opportunity").attr('style','font-weight:bold !important');;
		}else if(obj.type=="internal"){
			$('#lead_name').text("Internal").attr('style','font-weight:bold !important');;
		}else if(obj.type=="support"){
			$('#lead_name').text("Ticket").attr('style','font-weight:bold !important');;
		}
		if(obj.cancel_remarks){
			$(".cancel_column").show();
			$("#cancelation_remark").text(obj.cancel_remarks);
		}else{
			$(".cancel_column").hide();
			$("#cancelation_remark").text("");
		}
		var star_time = moment(obj.meeting_start);
		var end_time = moment(obj.meeting_end);		
		var cal_duration = moment.duration(moment(end_time, 'YYYY-MM-DD HH:mm:ss').diff(moment(star_time, 'YYYY-MM-DD HH:mm:ss'))).asMilliseconds("");
		var duration = moment.utc(cal_duration).format("HH:mm:ss");
		var phone_num = JSON.parse(obj.employeephone1), phone_row = "";
		for(i = 0; i < phone_num.phone.length; i++){
			if((i + 1) ==  phone_num.phone.length){
				phone_row += phone_num.phone[i];
				if(phone_num.phone[i] == ""){
		...
		/* $("#phone3").empty().append(phone_row); */
				
		/* $('#reschd1').text(obj.lead_reminder_id); */
		/* $("#act3").text(obj.lookup_value);
		$("#contact4").text(obj.employeename);
		$("#lead4").text(obj.leadname);
		$("#start_date3").text(moment(obj.meeting_start).format('lll'));
		$("#start_time3").text(obj.rem_time);
		$("#actve_duration3").text(duration);
		$("#reminder_time3").text(obj.addremtime);
		$("#note3").text(window.atob(obj.remarks)); */
	}
	...
	*************************** reschedule Submitting reschedule data **********************************
	...
	function reschedule_popup()	{
		...
		$.ajax({ 
			type : "POST",
			url : "<?php echo site_url('manager_mytaskController/update_reschedule'); ?>",
			dataType : 'json',
			data : JSON.stringify(addEventObj),					
			cache : false,
			success : function(data){
	...
	<body class="hold-transition skin-blue sidebar-mini my-task-page">  
		...
		<div class="content-wrapper body-content">
				...
				<div class="row header1">
					<div class="col-xs-4 col-sm-4 col-md-4 col-lg-4 aa">
						...
						<div class="style_video">
							<span class="glyphicon glyphicon-facetime-video " onclick="select_video('Team_Tasks', 'video_body', 'Manager')" data-toggle="tooltip" data-placement="right" title="Click to play info video"></span>
						</div>
					</div>
			...
			<ul class="nav nav-tabs" id="active_color">
				<li class="active" onclick="pageload()" id="scheduled_task_tab"><a data-toggle="tab" href="#open">Open</a></li>
				<li id="completed_reshcedule_task_tab"><a data-toggle="tab" href="#close" onclick="completed_data(0)">Closed</a></li>				
			  </ul>
						...
						<form class="form" action="#" method="post" name="adminClient">
							...
							<div class="modal-body">	
								...
								<div class="col-md-1 col-sm-1 col-xs-1" ></div>
									...
									<div class="col-md-8 col-sm-8 col-xs-8">
										<input type="text" id="actve_duration2" class="form-control" placeholder="HH:MM" readonly>									
										<span class="error-alert"></span>																		
									</div>
									<div class="col-md-1 col-sm-1 col-xs-1" ></div>
								</div>	
			...
			<div id="reschedule1" class="modal fade" data-backdrop="static">
				<div class="modal-dialog">
					<div class="modal-content">
						<div class="modal-header ">
							 <span class="close" onclick="cancel()">x</span>
							 <h4 class="modal-title text-center"><label id="event_name3"></label></h4>
						</div>
						<div class="modal-body">	
							<!--<div class="row">
								<div class="col-md-2 col-sm-1 col-xs-1" ></div>
							...
							<div class="row">
								<div class="col-md-2 col-sm-1 col-xs-1" ></div>
								<div class="col-md-3 col-sm-3 col-xs-3" >
									<label  id="lead_name"><b>Lead</b></label>
								</div>
								<div class="col-md-5 col-sm-5 col-xs-5">
							...
							<div class="row cancel_column none">
								...
								<div class="col-md-3 col-lg-3 col-sm-3 col-xs-3">										
									<label><b>Cancelation Remarks</b></label>
								</div>
								<div class="col-md-5 col-sm-5 col-xs-5">											
					...
					<div class="modal-content mytask_add_modal">
						<form class="form" action="#" method="post" name="adminClient">
							...
							<div class="modal-body">
                                ...
                                <div class="row team_mem_show">
									<div class="col-md-1"></div>
									<div class="col-md-2" >										
										<label for="activity">Activity Owner*</label>
									</div>
									<div class="col-md-8">
										<select class="form-control" id="team_members" style="display:none">
											
										</select>
										<select class="form-control" id="team_members1" >
								...
								<div class="row">
									...
									<div class="col-md-8">
										<input type="text" id="actve_duration" class="form-control" placeholder="HH:MM"  readonly>			
										<span class="error-alert"></span>
									</div>
								</div>
								<div class="row">
...
<!-- 								<div class="row">
								...
								<div class="row act_dur">
										...
										<div class="col-md-2">										
											...
											<div class="col-md-8">
												<input type="text" id="cmp_duration" class="form-control" placeholder="HH:MM" maxlength="5" readonly>									
												<span class="error-alert"></span>
											</div> 
											<div class="col-md-1">
											</div>
