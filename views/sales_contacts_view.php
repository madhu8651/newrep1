...
<head>
	...
	<script>
		var sales_contacts_leads;
		var sales_contacts_customers;
		var sales_contacts_contact_types;

		$(document).ready(function()	{
			pageload();
			$('#imageUploadA1').on('submit', function (e) {
				e.preventDefault();
			});
		...
		function pageload()	{
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('sales_contactListController/initView'); ?>",
				dataType : 'json',
				cache : false,
				...
				success : function(data){
					...
					sales_contacts_leads = data.leads;
					sales_contacts_customers = data.customers;
					setLeads(data.leads);
					sales_opportunity = data.opportunity;
					sales_contacts_contact_types = data.contact_type;
		...
		function changeTarget(obj) {
			...
			if (target == 'lead') {
				...
				setLeads(sales_contacts_leads);
			} else if (target == 'customer') {
				$('#target_placeholder_add').text('Customer ');
				setCustomers(sales_contacts_customers);
			} else if (target == 'opportunity') {
				$('#target_placeholder_add').text('opportunity ');
				setCustomers(sales_opportunity , 'opportunity');
			}
		}
		...
		function add() {
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('sales_contactListController/add_leadContact'); ?>",
				dataType : 'json',
		...
		function edit() {
			...
			*/
			...
			$.ajax({
				type : "POST",
				url : "<?php echo site_url('sales_contactListController/edit_leadContact'); ?>",
				dataType : 'json',
		...
		function contactImageLoaded(param) {
			...
			var imgid = '';
			if (param == 'add'){
				input = document.getElementById("imageInputAdd1");
		...
		function contactUploadImage(contact_id, sender) {
			...
			var formData;
			if (sender == 'add') {
				formData = new FormData($('#imageUploadA1')[0]);
			} else if (sender == 'edit') {
			...
			$.ajax({
				...
				url: "<?php echo site_url("sales_contactListController/file_upload/"); ?>"+contact_id,
				data: formData,
				dataType : 'json',
				cache: false,
				contentType: false,
