	...
	function complete_alert(){
		...
		$.ajax({ 	
			...
			success : function(data){
				...
				time_overlap = false;							
				$("#event_id_hidden").val(obj.leadempid);							
				$("#lemp_id").val(obj.leadempid);							
...
$.ajax({
	...
	success :function(data){
		$.ajax({
        type : "POST",
        url : "<?php echo site_url('manager_calendarController/initCal'); ?>",
        dataType : 'json',
        cache : false,
        success : function(data){	
			...
			$('#calendar').fullCalendar({
				...
				completed_value = event;
					...
					if(event.status!='complete' && event.status!='cancel' && event.status!='reschedule'){	
						...
						/* input.leadempid = event.leadempid; */
									  ...
									  <div class="modal-dialog">
										<div class="modal-content">
										  <div class="modal-header" style="padding-bottom: 35px;">
											...
											<span onclick="reschedulepopup('`+event.id+`','`+event.title+`','`+event.activity_id+`','`+event.activity_name+`','`+event.activity_owner+`','`+event.leadname+`','`+event.leadid+`','`+event.employeename+`','`+event.employeeid+`','`+event.start._i+`','`+event.duration+`','`+event.remarks+`','`+event.addremtime+`','`+event.person_id+`','`+event.created_by+`','`+event.type+`','`+event.reminder_members+`')" title="Reschedule" class="glyphicon glyphicon-time icon-big"></span> 
										  </div>  
										  ...
										  <div class="row">
											  <div>
												<center>
													<span class="task_comp none"><input type='checkbox' id='confirm_process1' onchange="confirm_process(1000, '`+event.start._i+`')"><label for='confirm_process1'>Task Completed</label></span>
						...
						******************* checking loggedin user id and event owner id****************************
						------------------------------------------------------------------------------------------*/	
						if("<?php echo $this->session->userdata('uid');?>" == event.person_id){
							if(assigned_date == CurrentDate){
								$(".task_comp").show();
							}else if(assigned_date < CurrentDate){
								$(".task_comp").show();
							}								
						}else{
							$(".task_comp").hide();
	...
	function confirm_process(fotmtype, time){
		if(fotmtype == "add"){
			...
			if($("#confirm_processAdd").prop('checked')==true){	
			    ...
			    $(".rating_msg ").text("");
			    $(".email_section, .email_alert").hide();
			    $("#email_list").html("");
	...
	**************************** save button click -- When Completed Task checkbox is not checkex ****************************************
	...
	function addevent(a)	{
		...
		if($.trim($("#start_date").val()) == ""){
			$("#start_date").closest("div").siblings("span").text("Start date is required.");
			return;	
		} else 	{
			date = moment($.trim($("#start_date").val()), 'lll').format('YYYY-MM-DD HH:mm:ss');
			$("#start_date").closest("#startDateTimePicker").siblings("span").text("");
		}
		...
		addEventObj.reminder_members = email_members;
	...
	***********************************************************************
	...
	function processComplete(showAddModel){
		...
		addEventObj.camp_note = 		$.trim($("#cmp_note").val());			/* -- event remarks */
						...
						<form id="addpopup" class="form" action="#" method="post" name="adminClient">
							...
							<div class="modal-body">
                                ...
                                <div class="row team_mem_show">
									...
									<div class="col-md-8">
										<input type="hidden" id="close_task" />
