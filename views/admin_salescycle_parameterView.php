...
function loadpage(){
        ...
        $.ajax({
            type : "POST",
            url : "<?php echo site_url('admin_salescycle_parameterController/display_data'); ?>",
            dataType : 'json',
            cache : false,
                  ...
                  if(data[i].cycle_togglebit == 0){
                       var str="";
                        ...
                        for(var a=0;a < data[i]['param_data'].length; a++){
                          if(data[i].param_data[a].parameter_for =="new_sell")
                          {selltype="New Sell";str4+='<li>'+ selltype +'</li>';}
                          if(data[i].param_data[a].parameter_for =="up_sell")
                          {selltype="Up Sell";str4+='<li>'+ selltype +'</li>';}
                          if(data[i].param_data[a].parameter_for =="cross_sell")
                          {selltype="Cross Sell";str4+='<li>'+ selltype +'</li>';}
                          if(data[i].param_data[a].parameter_for =="renewal")
                          {selltype="Renewal";str4+='<li>'+ selltype +'</li>';}
...
function compose(){
        ...
        $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_salescycle_parameterController/product'); ?>",
			dataType : 'json',
			cache : false,
			success : function(data){
				if(error_handler(data)){
...
var parameter_industry ="", parameter_location ="";
...
function selrow(obj,type){
    ...
    loaderShow();
        ...
        for(var i=0;i<obj.param_data.length;i++){
            ...
            }else if(obj.param_data[i].parameter_for == 'renewal'){
               $("#renewalE").prop('checked',true);
            }
        }
    $("#hid_type").val(type);
    ...
    addObj.cycle_id=obj.cycle_id;
     ...
     /* ----------------------  product tree*/
      $.ajax({
          type : "POST",
          url : "<?php echo site_url('admin_salescycle_parameterController/get_productdata_edit'); ?>",
          dataType : 'json',
...
function sectionOneP(status){
	...
	if(status == 'add'){
            ...
            if(myArrayID_pro.length  == 0){
                $(".error-alert.product-list").text("Please Choose the Products for the Sales Cycle");
                loaderHide();
                return;
            }else{
			...
			if(lastnodeFlag == 0){
    		 ...
    		 $.ajax({
    			type : "POST",
    			url : "<?php echo site_url('admin_salescycle_parameterController/industry'); ?>",
    			dataType : 'json',
    			cache : false,
    			success : function(data){
					if(error_handler(data)){
    ...
    if(status == 'edit'){
        ...
        lastnodeFlag=0;
            ...
            addObj.paramid=$("#hid_cycleid").val();
    		 $.ajax({
    			type : "POST",
    			url : "<?php echo site_url('admin_salescycle_parameterController/get_industry_edit'); ?>",
    			dataType : 'json',
    			data : JSON.stringify(addObj),
    			cache : false,
    			success : function(data){
...
function sectionTwoP(status){
	...
	if(status == 'add'){
		...
		if(industryArray.length  == 0){
			$(".error-alert.industry-list").text("Please Choose an Industry for Sales Cycle.");
            loaderHide();
			return;
		}else{
		...
		if(lastnodeFlag == 0){
		 ...
		 $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_salescycle_parameterController/locations'); ?>",
			dataType : 'json',
			cache : false,
			success : function(data){
				if(error_handler(data)){
    ...
    if(status == 'edit'){
        ...
        addObj.paramid=$("#hid_cycleid").val();
		 $.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_salescycle_parameterController/get_business_edit'); ?>",
			dataType : 'json',
			data : JSON.stringify(addObj),
			cache : false,
			success : function(data){
...
function sectionThreeP(status){
	...
	if(status == 'add'){
		...
		if(BusinessLocationArray.length  == 0){
			$(".error-alert.business-list").text("Please Choose an Business Location for Sales Cycle.");
            loaderHide();
			return;
		}else{
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('admin_salescycle_parameterController/get_cycle'); ?>",
			dataType : 'json',
			cache : false,
			success : function(data){
				if(error_handler(data)){
				...
				for(var i=0;i<data.length; i++)
				{
					 options += "<option value='"+data[i].CYCLE_ID+"_"+data[i].togglebit+"'>"+ data[i].CYCLE_NAME +"</option>";
				}
...
function sectionFourP(status){
  ...
  if(status == 'add'){
        ...
        if($("#salescyle").val()==""){
      	    $("#salescyle").closest("div").find("span").text("Sales Cycle is required.");
              loaderHide()
      	    return;
        }else{
		...
		var seltypeChk=0;
		$("#sales_newsell").closest(".row").find("input[type=checkbox]").each(function(){
			if($(this).prop('checked') == true){
				seltypeChk=1;
			}
		});
		if(seltypeChk==0){
         ...
         common_fn_togetCombination("display_prod","display_loc","display_Category","sales_newsell","sales_upsell","sales_crosssell","renewal","addmodal");
  }
  /* ------------------edit module-------------------------- */
  if(status == 'edit'){
			...
			var seltypeChk=0;
			$("#sales_newsellE").closest(".row").find("input[type=checkbox]").each(function(){
				if($(this).prop('checked') == true){
					seltypeChk=1;
				}
			});
			if(seltypeChk==0){
				$('#seltypeErchkE span').text("Please choose atleast one Sell Type");
				return;
			}else{
       ...
       common_fn_togetCombination("display_prodE","display_locE","display_CategoryE","sales_newsellE","sales_upsellE","sales_crosssellE","renewalE","editmodal");
  }
...
function common_fn_togetCombination(pro_listid,industry_listid,busi_listid,nwsell_id,upsell_id,cross_id,renewalid,modal_type){
    var selectedProd=[];
    ...
    $("#"+nwsell_id+", #"+upsell_id+", #"+cross_id+", #"+renewalid+"").each(function(){
    	if($(this).prop('checked')== true){
    		selectedsallType.push($(this).val());
    	}
    });
    ...
    /* -------------------------------------------------- */
    var allArrays = new Array(listProdName,listIndsName,listBusiName,selectedsallType);
    var allIdArrays = new Array(listProdId,listIndsId,listBusiId,selectedsallType);
    ...
    var str = "";
    var str21 = "";
    str += "<div id='combination'><table class='table'><thead><tr><th>Sl No</th><th>Products</th><th>Industry</th><th>Business</th><th>Sell Type</th></tr></thead><tbody>";
    for(i=0;i<result1.length;i++){
        ...
        if(Idval[3] == 'new_sell'){
              val[3]='New Sell';
        }else if(Idval[3] == 'cross_sell'){
              val[3]='Cross Sell';
        }else if(Idval[3] == 'up_sell'){
              val[3]='Up Sell';
        }
		if(i<10){
...
function checkDuplicate(status){
	...
	if(status == 'add' || typeofsave == 'addrow'){
		...
		$("#combination table tr input[type=checkbox]").each(function(){
			if($(this).prop('checked')== true){
                ...
                NmArray1.push($(this).closest('tr').find('.txt01').text()+','+$(this).closest('tr').find('.txt02').text()+','+$(this).closest('tr').find('.txt03').text()+','+$(this).closest('tr').find('.txt04').text());
			}
		});
        ...
        $.ajax({
                    type : "POST",
                    url : "<?php echo site_url('admin_salescycle_parameterController/checkDuplicate/add'); ?>",
                    dataType : 'json',
                    data : JSON.stringify(addObj),
                    cache : false,
                    success : function(data){
    ...
    if(status == 'edit' ){
        ...
        $.ajax({
                    type : "POST",
                    url : "<?php echo site_url('admin_salescycle_parameterController/checkDuplicate/update'); ?>",
                    dataType : 'json',
                    data : JSON.stringify(addObj),
                    cache : false,
                    success : function(data){
...
function add_cycle(){
    ...
    if(myArrayID_pro.length  == 0){
        $("#products").siblings(".error-alert").text("Please Choose the Products for the Sales Cycle");
        loaderHide();
        return;
    }else{
    ...
    var listCombId="";
      ...
      $.ajax({
                type : "POST",
                url : "<?php echo site_url('admin_salescycle_parameterController/post_data'); ?>",
                dataType : 'json',
                data : JSON.stringify(addObj),
                cache : false,
                success : function(data){
...
function saveedit(){
    ...
    if(myArrayID_pro.length  == 0){
        $("#productsE").siblings(".error-alert").text("Please Choose the Products for the Sales Cycle");
        loaderHide();
        return;
    }else{
    ...
    if(typeofsave =='editrow'){
                 $.ajax({
                   type : "POST",
                   url : "<?php echo site_url('admin_salescycle_parameterController/update_data'); ?>",
                   dataType : 'json',
                   data : JSON.stringify(addObj),
                   cache : false,
                   success : function(data){
			...
			<div class="row header1">
					<div class="col-xs-2 col-sm-2 col-md-4 col-lg-4 aa">
						<span class="info-icon">
							<div >
								<img src="<?php echo base_url(); ?>images/new/i_off.png" onmouseover="this.src='<?php echo base_url(); ?>images/new/i_ON.png'" onmouseout="this.src='<?php echo base_url(); ?>images/new/i_off.png'" alt="info" width="30" height="30"  data-toggle="tooltip" data-placement="right" title="Sales Cycle Parameters"/>
							</div>
						</span>
						<div class="style_video">
							<span class="glyphicon glyphicon-facetime-video " onclick="select_video('Sales_Cycle_Parameters', 'video_body', 'Admin')" data-toggle="tooltip" data-placement="right" title="Click to play info video"></span>
						</div>
					</div>
					<div class="col-xs-8 col-sm-8 col-md-4 col-lg-4 pageHeader1 aa">
							<h2>Sales Cycle Parameters</h2>
					</div>
					<div  class="col-xs-2 col-sm-2 col-md-4 col-lg-4  aa">
						 <div class="addBtns">
					...
					<table class="table">
						<thead>
							<tr>
                                ...
                                <th class="table_header" width="20%">Sales Cycle</th>
                    ...
                    <div class="modal-content">
                        <form id="editpopup" class="form" action="#" method="post" name="adminClient">
                            <div class="modal-header">
                                 ...
                                 <h4 class="modal-title">Edit Sales Cycle Parameters</h4>
                            </div>
							...
							<div class="modal-body section-four">
								<div class="row">
								  <div class="col-md-3">
									  <label for="edit_cycle">Sales Cycle*</label>
								  </div>
								  <div class="col-md-9">
									...
									<div class="row">
                                      ...
                                      <div class="col-md-9 no-padding">
                                            <div class="row">
            								    <div class="col-md-3">
            										<input  type="checkbox" id="sales_newsellE" value="new_sell"/>
            										<label for="sales_newsellE"> New Sell</label>
            								    </div>
            								    <div class="col-md-3 ">
            										<input type="checkbox"  id="sales_upsellE" value="up_sell"/>
            										<label for="sales_upsellE"> Up Sell</label>
            								    </div>
                                                <div class="col-md-3 ">
            										<input type="checkbox" id="sales_crosssellE" value="cross_sell"/>
            										<label for="sales_crosssellE"> Cross Sell</label>
            								    </div>
                                                <div class="col-md-3 ">
            										<input type="checkbox" id="renewalE" value="renewal"/>
            										<label for="renewalE">Renewal</label>
            								    </div>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <form id="addpopup" class="form" action="#" method="post" name="adminClient">
                            <div class="modal-header">
                                 <span class="close" onclick="cancel()">x</span>
                                 <h4 class="modal-title">Add Sales Cycle Parameters</h4>
                            </div>
                            <div class="modal-body section-one">
                                <div class="row">
                                    <div class="col-md-12 alert alert-info">
										<i><i class="fa fa-info-circle" aria-hidden="true"></i> <label for="products">Please choose a Product for the Sales Cycle *</label></i>
									</div>
									<div class="pull-left" >
							...
							<div class="modal-body section-two">
								<div class="row">
                                    <div class="col-md-12 alert alert-info">
										<i><i class="fa fa-info-circle" aria-hidden="true"></i> <label for="industry">Please choose an Industry for the Sales Cycle *</label></i>
									</div>
									<div class="pull-left">
                            ...
                            <div class="modal-body section-three">
								<div class="row">
                                    <div class="col-md-12 alert alert-info">
										<i><i class="fa fa-info-circle" aria-hidden="true"></i><label for="business_loc">Please choose a Business Location for the Sales Cycle *</label></i>
									</div>
									<div class="pull-left">
							...
							<div class="modal-body section-four">
								<div class="row">
                                    <div class="col-md-12 alert alert-info">
										<i><i class="fa fa-info-circle" aria-hidden="true"></i> Please choose Cycle and Sell Type for the Sales Cycle</i>
									</div>
    								<div class="col-md-3">
    									  <label for="salescyle">Sales Cycle*</label>
    								</div>
								    <div class="col-md-9">
								...
								<div class="row ">
								    ...
								    <div class="col-md-9 ">
    								    <div class="row no-padding">
                                            ...
                                            <div class="col-md-3 ">
        										<input type="checkbox" id="renewal" value="renewal"/>
