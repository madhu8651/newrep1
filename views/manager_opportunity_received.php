...
function loaddata(){
	$.ajax({
		type : "POST",
		url : "<?php echo site_url('manager_opportunitiesController/get_received_opportunities'); ?>",
		dataType : 'json',
		cache : false,
		success : function(data){
			...
			for(i=0; i < data.length; i++ ){
				...
				var ownership ='';
				if(data[i].owner_manager_status==1 || data[i].owner_manager_status==3){
					...
					if(data[i].ownerreject>0){
						...
						if(data[i].manager_owner_name=='pending'){
							ownership="<span><font color='#FF8C00'><b>Pending<b></font></span>"
						} else{
							ownership = data[i].manager_owner_name;
						}
					}
				}
				var stage_ownership ='';
				if(data[i].stage_manager_owner_status==1 || data[i].stage_manager_owner_status==3){
					...
					if(data[i].stagereject>0){
					   stage_ownership ="<span><font color='#cc0000'><b>Declined<b></font></span>";
						...
						stage_ownership = data[i].stage_manager_owner_name;
					}
				}
				var  link = "<?php echo site_url('manager_opportunitiesController/stage_view/'); ?>"+data[i].opportunity_id;
				row += "<tr>\
...
function accept_owner(obj,owner){
	...
	$.ajax({
		type : "POST",
		url : "<?php echo site_url('manager_opportunitiesController/accept_opportunity'); ?>",
...
function reject_opportunity(){
	...
	if(checkboxChkFlg==0){
		...
		$('#reject_stage_id').val(aa.stage_id);
		if(aa.owner_manager_status==1){
			$('#owner1').css({'display':'block'});
		}
		if(aa.stage_manager_owner_status==1){
...
function final_rejection(){
	...
	$.ajax({
		type : "POST",
		url : "<?php echo site_url('manager_opportunitiesController/reject_opportunity'); ?>",
		data    : JSON.stringify(addObj),
		dataType : 'json',
		cache : false,
		success : function(data){
		...
		<div class="row header1">
			<div class="col-xs-2 col-sm-2 col-md-3 col-lg-3 aa">
				<span class="info-icon toolTipStyle">
					<div >
				<img src="<?php echo site_url()?>/images/new/i_off.png" onmouseover="this.src='<?php echo site_url()?>/images/new/i_ON.png'" onmouseout="this.src='<?php echo site_url()?>/images/new/i_off.png'" alt="info" width="30" height="30"  data-toggle="tooltip" data-placement="bottom" data-html="true" title="Find all the opportunities assigned to you on this page. Once you accept an opportunity, it will either go to ‘Unassigned’ if an executive has not been assigned to it or it will go to ‘Assigned’ with the name of the executive assigned to the opportunity. Click on the <span class='glyphicon glyphicon-eye-open tooleye'></span> on each opportunity to view their details. Click on the <img src='<?php echo site_url(); ?>images/new/Plus_Off.png' width='20px' height='20px' /> button on the top right to add a new opportunity. Please note that you can only add new opportunities for leads that are in progress."/>
					</div>
				</span>
			</div>
			<div class="col-xs-8 col-sm-8 col-md-6 col-lg-6 pageHeader1 aa">
					<h2>Received Opportunities</h2>
			</div>
			<div class="col-xs-2 col-sm-2 col-md-3 col-lg-3 aa">
