...
function assignedLoad(){
	...
	$.ajax({
		type: "POST",
		url: "<?php echo site_url('sales_customerController/getAssignedCustomerDetails');?>",
		dataType:'json',
...
$("#edit_leadmail").val(JSON.parse(obj.customer_email));
$("#edit_leadphone").val(JSON.parse(obj.customer_number));
$("#edit_city").val(obj.customer_city);
...
$('#edit_selectmap').hide();
$("#edit_country").val(obj.customercountry);
$("#edit_state").val(obj.customerstate);
$('#edit_map1').show();
...
$.ajax({
 ...
 $.ajax({
        ...
        var select = $("#edit_country"), options = "<option value=''>select</option>";
           ...
           select.append(options);
           $("#edit_country option[value='"+obj.customercountry+"']").attr("selected",true);

        }
        });
        var id= obj.customercountry;
                $.ajax({ 
                type : "POST",
                ...
                success : function(data){
                    ...
                    var select = $("#edit_state"), options = "<option value=''>Select</option>";
                        ...
                        select.append(options);
                        $("#edit_state option[value='"+obj.customerstate+"']").attr("selected",true);
       ...
       $.ajax({
            ...
            currencyhtml +='<ul>';
                    ...
                    currencyhtml +='</div>';
                    $("#edit_product").append(currencyhtml)
                     $("#edit_product1").append(currencyhtml)
            }
        });
          ...
          $.ajax({
            ...
            success: function(data){
            	if(error_handler(data)){
                 ...
                 console.log(data);
            var currencyhtml1="";
            currencyhtml1 +='<div id="product_value2" class="multiselect">';
            currencyhtml1 +='<ul>';
                    for(var j=0;j<data.length; j++){								
    ...
    addObj.customer_id=$("#customer_id").val();
       $.ajax({
            ...
            success: function(data){ 
					...
					for (i = data.length-1;i>=0; i--) {
						var rowdata1 = JSON.stringify(data[i]);
						row += "<div id='"+data[i].purchase_id+"' style='border: 1px solid #d6cece;padding: 3px;'><div class='row product_heading' style='text-align:center'>Purchase Order "+(i+1)+"<span class='edit_purchase none'><a href='#' onclick='editselrow("+rowdata1+")'><span class='glyphicon glyphicon-pencil'></span></a></span></div><div class='row'><div class='col-md-2'><label>Product Owner</label></div><div class='col-md-4'>"+data[i].product_owner+"</div><div class='col-md-2'><label>Currency</label></div><div class='col-md-4'>"+data[i].currency_name+"</div></div><div class='row'><div class='col-md-2'><label>Reference Number</label></div><div class='col-md-4'>"+data[i].reference_number+"</div></div>";
						for(j=0;j<data[i].prod_data.length;j++){
							if(data.length-1==0){
								$(".prod_purchase_info").css("height", 184);
								$(".prod_purchase_info").css("margin-bottom", 8);
...
function assign_save(){
	...
	/* from the lending page */
	console.log($("#reject_save_section").val())
...
function assign_date(idval,note){
	...
	if(note == ""){
		$.ajax({
			type: "POST",
			url: "<?php echo site_url('sales_customerController/customerAccept'); ?>",
		...
		$.ajax({
            type: "POST",
            url: "<?php echo site_url('sales_customerController/customerReject'); ?>",
            data:JSON.stringify(arr),
            dataType:'json',
            success: function(data) {
				console.log(data)
							   ...
							   <div class="row" >
								...
								<div class="row">
									<ul class="nav nav-tabs">
										<li class="active" onclick="leadlog();" id="logdetails_cust">
											<a data-toggle="tab" href="#logdetails">Customer History</a>
										</li>
										<li onclick="schedule_fetch();">
											<a data-toggle="tab" href="#logdetails1">Scheduled Activities</a>
										</li>
										<li onclick="opp_log();">
											<a data-toggle="tab" href="#opp_details1">Opportunities</a>
										</li>
										<li onclick="hist_log();">
									...
									<div class="tab-content">
										<div id="logdetails" class="tab-pane fade in active"></div>
										<!------------------------------------------------------>
										<div id="logdetails1" class="tab-pane fade"></div>
										<!------------------------------------------------------>
										<div id="opp_details1" class="tab-pane fade"></div>
										<!------------------------------------------------------>
         ...
         <div id="modalstart1" class="modal fade" data-backdrop="static">
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <form id="addpopup" class="form">
                            	<input id="reject_save_section" type="hidden" />
                                <div class="modal-header">
