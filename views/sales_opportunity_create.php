...
*/	var leads;
		...
		$('#target_list').on('change',function(){
			...
			if ((id != '') && ((industry_id != '')&&(industry_id != null)) && ((location_id != '')&&(location_id != null))) {
				...
				*/
				...
				$.ajax({
					type : "POST",
					url : "<?php echo site_url('sales_opportunitiesController/get_contactsAndProducts/').$target; ?>",
				...
				$('#target_list').val("");
				$("#target_list").closest("div").find(".warning-alert").text("Check Industry and Location for selected Lead");
				$("#target_list").focus();
			}		
		});
		...
		$('#product_list').on('change',function(){
			...
			if (product.product_id && product.target_id) {
				...
				*/
				$.ajax({
					type : "POST",
					url : "<?php echo site_url('sales_opportunitiesController/get_sellType'); ?>",
					dataType : 'json',
					cache : false,
	...
	function sales_opp_create_pageload() {
		var target = "<?php echo $target; ?>";
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('sales_opportunitiesController/init/').$target; ?>",
			dataType : 'json',
			cache : false,
			success : function(data)	{
				if(error_handler(data)) {
	...
	function add()	{
		...
		$.ajax({
			type : "POST",
			url : "<?php echo site_url('sales_opportunitiesController/get_SalesCycle2'); ?>",
			dataType : 'json',
			cache : false,
			...
			success : function(data)	{
				...
				if (data.status != false) {
					...
					document.getElementById('createOppoForm').action = "<?php echo site_url('sales_opportunitiesController/stage_view/'); ?>"+opp_id;
			...
			<div>
				...
				<div class="col-md-12">
					<div class="col-md-6">
						<div class="row inputBottomGap">
							<div class="col-md-3">
								<label for="target_list" id="target_placeholder">Name</label> 
							</div>
							<div class="col-md-8">
					...
					<div class="col-md-6">
						...
						<div class="row inputBottomGap">
							...
							<div class="col-md-8">
								<textarea rows="4" cols="50" placeholder="Enter additional remarks for the Opportunity" class="form-control" name="opportunities_remarks" id="opportunities_remarks"></textarea>
								<span class="error-alert"></span>
							</div>
						</div>
					</div>
				...
				<div class="row">
					<center>
						...
						<a href="<?php echo site_url('sales_opportunitiesController/new_opportunities');?>"> 
