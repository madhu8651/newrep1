...
function loaddata(){
    $('#tablebody').parent("table").dataTable().fnDestroy();
      $.ajax({
            type: "POST",
            url: "<?php echo site_url('sales_supportController/get_ticketDetails'); ?>",
            dataType:'json',
...
function request_accept(status){
   ...
   loaderShow();
     $.ajax({
    type : "POST",
    url : "<?php echo site_url('sales_supportController/accept_multiple'); ?>",
    dataType: "json",
...
function final_rejection(){
    ...
    $.ajax({
        type : "POST",
        url : "<?php echo site_url('sales_supportController/reject_multiple'); ?>",
        dataType: "json",
        data    : JSON.stringify(addObj),
        cache : false,
        success : function(data){
                    ...
                    <div class="modal-dialog ">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <button  type="button" class="btn btn-default" id="reassign" onclick="request_accept('single')" >Accept</button>
                                <button  type="button" class="btn btn-default" id="btn1_cancel" onclick="cancel1()" >Cancel</button>
                            </div>
                        </div>
                    </div>
                </div> 
               <?php require 'sales_edit_request_modal.php' ?>
