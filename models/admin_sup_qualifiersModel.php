...
$GLOBALS['$log'] = Logger::getLogger('admin_sup_qualifiersModel');
$GLOBALS['$dbFramework'] = new LConnectDataAccess();


class admin_sup_qualifiersModel extends CI_Model{
    ...
    public function view_data(){
        try{
            $query=$GLOBALS['$dbFramework']->query("select * from lead_qualifier where lead_qualifier_type='support' order by id ");
            ...
            for($i=0;$i<count($arr);$i++){
                  ...
                  $query1=$GLOBALS['$dbFramework']->query("select * from support_sales_stage_attributes where attribute_value='$lead_qualifier_id' ");
                  $arr1=$query1->result_array();
                      for($j=0;$j<count($arr1);$j++){
                          $stage_id=$arr1[$j]['stage_id'];

                          $query11=$GLOBALS['$dbFramework']->query("select c.CYCLE_NAME,c.CYCLE_ID,a.stage_name,a.stage_id from support_sales_stage a,support_stage_cycle_mapping b,
                                              support_sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.CYCLE_ID and a.stage_id='$stage_id'
    ...
    public function insert_data($data,$qualifiername){
        try{
              ...
              //$query=$GLOBALS['$dbFramework']->query("call common_check_duplicate('lead_qualifier','lead_qualifier_name','".ucfirst(strtolower($qualifiername))."','lead_qualifier_type','support')");
              $query=$GLOBALS['$dbFramework']->query("select * from lead_qualifier where LOWER(lead_qualifier_name)=LOWER('".$qualifiername."') AND lead_qualifier_type='support'");
              if($query->num_rows()>0){
                return false;
              }
              else{
    ...
    public function update_qualifiername($data,$qualifiername,$qualifierid){
        try{
              ...
              //$query=$GLOBALS['$dbFramework']->query("call common_check_duplicate('lead_qualifier','lead_qualifier_name','".ucfirst(strtolower($qualifiername))."','lead_qualifier_type','support')");
              $query=$GLOBALS['$dbFramework']->query("select * from lead_qualifier where LOWER(lead_qualifier_name)=LOWER('".$qualifiername."') AND lead_qualifier_type='support' and lead_qualifier_id<>'$qualifierid'");
              if($query->num_rows()>0){
                return false;
              }
              else{
              ...
              if($query->num_rows()>0){
                return false;
                  ...
                  foreach ($ansdata as  $value) {
                      ...
                      //$upque="";
                      //$GLOBALS['$dbFramework']->query($upque);
