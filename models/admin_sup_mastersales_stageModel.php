...
$GLOBALS['$log'] = Logger::getLogger('admin_sup_mastersales_stageModel');
$GLOBALS['$dbFramework'] = new LConnectDataAccess();


class admin_sup_mastersales_stageModel extends CI_Model{
    ...
    public function get_data(){
                    ...
                    $dupbit=0;
            try{
                    $query=$GLOBALS['$dbFramework']->query("select * from support_master_sales_cycle order by id ");
                    ...
                    for($i=0;$i<count($arr);$i++){
                        ...
                        $query=$GLOBALS['$dbFramework']->query("select c.* from support_sales_stage a,support_stage_cycle_mapping b,
                                                                support_master_sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.master_cycleid
        ...
        public function get_data2($cycleid){
                    $b=array();
            try{
                    $query=$GLOBALS['$dbFramework']->query("select * from support_master_sales_cycle where master_cycleid ='$cycleid'");
                    $arr=$query->result_array();
                    for($i=0;$i<count($arr);$i++){
                        ...
                        $query1=$GLOBALS['$dbFramework']->query("select a.id,a.stage_id,a.stage_name,b.remarks from support_sales_stage a,support_stage_cycle_mapping b,
                                                    support_master_sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.master_cycleid and  c.master_cycleid='$CYCLE_ID' and a.stage_sequence >5 and a.stage_sequence<100 order by a.stage_sequence");
    ...
    public function insert_data($data1,$data2,$stageName,$cycleId,$new_stageid) {
        ...
        try{
                $str="";
                $query = $GLOBALS['$dbFramework']->query("select a.stage_name from support_sales_stage a,
                                                            support_stage_cycle_mapping b where a.stage_id=b.stage_id and
                ...
                $b=array();
                $query=$GLOBALS['$dbFramework']->query("select c.master_cyclename,c.master_cycleid from support_sales_stage a,support_stage_cycle_mapping b,
                          support_master_sales_cycle c where a.stage_id=b.stage_id and  b.cycle_id=c.master_cycleid and c.master_cycleid='$cycleId' and a.stage_sequence >5 and a.stage_sequence<100 group by c.master_cycleid,c.master_cyclename ");
                $arr=$query->result_array();
    ...
    public function update_data($stage_id,$stage_name,$edit_cycleid,$description){
            try{
                    $str="";
                    $query = $GLOBALS['$dbFramework']->query("select a.stage_name,a.stage_id from support_sales_stage a,
                                                                support_stage_cycle_mapping b where a.stage_id=b.stage_id and
                    ...
                    if($query->num_rows()>0){
                        ...
                        if(strtolower($stage_id1) <> strtolower($stage_id)){
                            $str="0";
                             ...
                             $str="1";
                             $GLOBALS['$dbFramework']->query("update support_stage_cycle_mapping set remarks='$description' where  trim(lower(stage_id))=trim(lower('$stage_id'))");
                        }
                    ...
                    $b=array();
                    $query=$GLOBALS['$dbFramework']->query("select c.master_cyclename,c.master_cycleid from support_sales_stage a,support_stage_cycle_mapping b,
                              support_master_sales_cycle c where a.stage_id=b.stage_id and  b.cycle_id=c.master_cycleid and c.master_cycleid='$edit_cycleid' and a.stage_sequence >5 and a.stage_sequence<100 group by c.master_cycleid,c.master_cyclename ");
                    $arr=$query->result_array();
    ...
    public function update_roworder($orderselected,$hid_cycleid){
        try{
                ...
                foreach ($orderselected as  $value) {
                     ...
                     $que=$GLOBALS['$dbFramework']->query("select stage_id from support_sales_stage where id='$row' ");
                     if($que->num_rows()>0){
                     ...
                     $updateque2=$GLOBALS['$dbFramework']->query("update support_stage_cycle_mapping set mapseq='$j' where master_stageid='$mstageid'");
    ...
    public function get_update_data($cycle_id){
            ...
            try{
                    $query=$GLOBALS['$dbFramework']->query("select c.master_cyclename,c.master_cycleid from support_sales_stage a,support_stage_cycle_mapping b,
                              support_master_sales_cycle c where a.stage_id=b.stage_id and  b.cycle_id=c.master_cycleid and c.master_cycleid='$cycle_id' and a.stage_sequence >5 and a.stage_sequence<100 group by c.master_cycleid,c.master_cyclename ");
                    $arr=$query->result_array();
                    for($i=0;$i<count($arr);$i++){
                        $CYCLE_ID=$arr[$i]['master_cycleid'];
    ...
    public function get_cycle(){
        try{
                $que=$GLOBALS['$dbFramework']->query("select c.master_cyclename as CYCLE_NAME,c.master_cycleid as CYCLE_ID from support_sales_stage a,support_stage_cycle_mapping b,
                                                  support_master_sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.master_cycleid
        ...
        try{
                   ...
                   if($que1->num_rows()>0){
                    ...
                    for($i=0;$i<count($arr);$i++){
                        ...
                        $query1=$GLOBALS['$dbFramework']->query("select a.id,a.stage_id,a.stage_name,b.remarks from support_sales_stage a,support_stage_cycle_mapping b,
                                                    support_master_sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.master_cycleid and  c.master_cycleid='$CYCLE_ID'
