...
$GLOBALS['$log'] = Logger::getLogger('admin_blocation_hierarchyModel');
$GLOBALS['$dbFramework'] = new LConnectDataAccess();


class admin_blocation_hierarchyModel extends CI_Model{
    ...
    public function insert_hierarchy($param,$source,$parent_id) {
        try{
                ...
                //$que=$GLOBALS['$dbFramework']->query("call get_hierarchy_details('business_location','insert','".ucfirst(strtolower($source))."','".$parent_id."');");
                $que=$GLOBALS['$dbFramework']->query("SELECT b.id,b.hierarchy_id,b.hkey2,b.hvalue2,b.hkey1,b.hvalue1
                                        				...
                                        				WHERE a.Hierarchy_Class_Name='business_location'
    ...
    public function get_hierarchy_id(){
        try{
            $query = $GLOBALS['$dbFramework']->query("select Hierarchy_Class_ID from hierarchy_class where Hierarchy_Class_Name='business_location'");
            return $query->result();
        }catch (LConnectApplicationException $e){
            $GLOBALS['$logger']->debug('!!!Exception Thrown to Model --- Passing to Controller!!!');
            throw $e;
    ...
    public function update_source($param,$id,$parent_id,$hid) {
        ...
        try{
                ...
                //$que=$GLOBALS['$dbFramework']->query("call get_hierarchy_details('business_location','update','".ucfirst(strtolower($param))."','".$parent_id."');");
                $que=$GLOBALS['$dbFramework']->query("SELECT b.id,b.hierarchy_id,b.hkey2,b.hvalue2,b.hkey1,b.hvalue1
                                        				FROM hierarchy_class a,hierarchy b
                                        				WHERE a.Hierarchy_Class_Name='business_location'
                                        				AND b.hkey2!='0' AND b.Hierarchy_Class_ID=a.hierarchy_class_id
