...
$GLOBALS['$log'] = Logger::getLogger('admin_sup_salescycle_parameterModel');
$GLOBALS['$dbFramework'] = new LConnectDataAccess();


class admin_sup_salescycle_parameterModel extends CI_Model{
...
public function view_data(){
    ...
    $a=array();
    try{
            $query=$GLOBALS['$dbFramework']->query("select distinct a.parameter_id,
                                    ...
                                    a.cycle_id,(select CYCLE_NAME from support_sales_cycle where CYCLE_ID=a.cycle_id) as cyclename,
                                    a.cycle_togglebit from support_sales_cycle_parameters a order by a.id ");
                    ...
                    for($i=0;$i<count($arr);$i++){
                            ...
                            $query1 = $this->db->query("select distinct a.parameter_product_node,
                                                        (select distinct hvalue2 from hierarchy where hkey2=a.parameter_product_node) as product
                                                            from support_sales_cycle_parameters a where cycle_id='".$cycle_id."';");
                             ...
                             $query1 = $this->db->query("select distinct a.parameter_for,(select lookup_value from lookup where lookup_id=a.parameter_for and togglebit=1)as paramname
                                                            from support_sales_cycle_parameters a where cycle_id='".$cycle_id."';");
                             if($query1->num_rows()>0){
                                    ...
                                    for($j=0;$j<count($arr1);$j++){
                                            ...
                                            $a[$i]['param_data'][$j]['paramname']=$arr1[$j]['paramname'];
...
public function cycle_data(){
    try{
            $a=$b=array();
            $query=$GLOBALS['$dbFramework']->query("select * from support_sales_cycle where togglebit=1 order by id ");
            $a= $query->result();

            $query1=$GLOBALS['$dbFramework']->query("select * from lookup WHERE lookup_name='support_process' and togglebit=1 and lookup_id not in ('new_sell','up_sell','cross_sell','renewal') order by id ");
            $b= $query1->result();
...
public function insert_data($data) {
    try{
            $var = $GLOBALS['$dbFramework']->insert_batch('support_sales_cycle_parameters',$data);
            return $var;
    }catch (LConnectApplicationException $e){
                $GLOBALS['$logger']->debug('!!!Exception Thrown to Model --- Passing to Controller!!!');
                throw $e;
...
public function industry_edit($paramid){
            ...
            try{
                    $query = $GLOBALS['$dbFramework']->query("call edit_tree('industry','support_page','$paramid','Editindus');");
                    if($query->num_rows()>0){
                        $json=$query->result();
...
public function business_edit($paramid){
            ...
            $c=array();
            try{
                    $query = $GLOBALS['$dbFramework']->query("call edit_tree('business_location','support_page','$paramid','Editloc');");
                    if($query->num_rows()>0){
                        $json=$query->result();
...
public function product_edit($paramid){
            ...
            $c=array();
            try{
                    $query = $GLOBALS['$dbFramework']->query("call edit_tree('products','support_page','$paramid','Editprod');");
                    if($query->num_rows()>0){
                        $json=$query->result();
...
public function update_data($maindata1,$hid_parameterid) {
    try{
            $delque=$GLOBALS['$dbFramework']->query("delete from support_sales_cycle_parameters where parameter_id='$hid_parameterid'");
            $var = $GLOBALS['$dbFramework']->insert_batch('support_sales_cycle_parameters',$maindata1);
            return $var;
    }catch (LConnectApplicationException $e){
                $GLOBALS['$logger']->debug('!!!Exception Thrown to Model --- Passing to Controller!!!');
                throw $e;
...
public function delDuplicate($listCombId) {
    try{
                ...
                $var = $GLOBALS['$dbFramework']->query("call SP_SPLIT('$listCombId',':','support',0);");
                return $var;
...
public function checkDuplicate($listCombId,$status,$hid_parameterid){
    ...
    try{
          if($status=="add"){
              $output=$GLOBALS['$dbFramework']->query("call SP_SPLIT('$listCombId',':','support',1);");
                  $arr=$output->num_rows();
                  if($arr > 0){
                  ...
                  $output=$GLOBALS['$dbFramework']->query("call UPDATE_SP_SPLIT('$listCombId','$hid_parameterid',':','support',1);");
                  $arr=$output->num_rows();
                  if($arr > 0){
                      return $output->result();
                  }else{
