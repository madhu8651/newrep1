...
$GLOBALS['$log'] = Logger::getLogger('admin_sales_stageModel');
$GLOBALS['$dbFramework'] = new LConnectDataAccess();


class admin_sales_stageModel extends CI_Model{
    ...
    public function get_data(){
        ...
        try{
                    ...
                    $oppquery=$GLOBALS['$dbFramework']->query("select distinct a.cycle_id,b.CYCLE_NAME,b.togglebit from stage_cycle_mapping a,sales_cycle b
                                                where a.cycle_id=b.CYCLE_ID   and a.cycle_id  in (select cycle_id from opportunity_details where a.cycle_id=cycle_id)");    // check the opportunity against cycle id
                    if($oppquery->num_rows()>0){
                            ...
                            for($row=0;$row<count($rowdata);$row++){
                                ...
                                $query=$GLOBALS['$dbFramework']->query("select c.CYCLE_NAME,c.CYCLE_ID from sales_stage a,stage_cycle_mapping b,
                                          sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.CYCLE_ID and b.cycle_id='$oppCYCLE_ID'  and a.stage_sequence >5 and a.stage_sequence<100 group by c.CYCLE_ID,c.CYCLE_NAME ");
                                if($query->num_rows()>0){
                                      $dupbit=1;
                                }
                    ...
                    $oppquery=$GLOBALS['$dbFramework']->query("select distinct a.cycle_id,b.CYCLE_NAME,b.togglebit from stage_cycle_mapping a,sales_cycle b
                                                where a.cycle_id=b.CYCLE_ID  and a.cycle_id not in (select cycle_id from opportunity_details where a.cycle_id=cycle_id)");    // check the opportunity against cycle id
                    if($oppquery->num_rows()>0){
    ...
    public function get_data2($cycleid){
        ...
        try{
                    /* cycle present in opportunity */
                    $oppquery=$GLOBALS['$dbFramework']->query("select distinct cycle_id from stage_cycle_mapping where cycle_id ='$cycleid'");    // check the opportunity against cycle id
                    if($oppquery->num_rows()>0){
                            ...
                            for($row=0;$row<count($rowdata);$row++){
                                ...
                                for($i=0;$i<count($arr);$i++){
                                    ...
                                    $query1=$GLOBALS['$dbFramework']->query("select a.id,a.stage_id,a.stage_name,b.remarks,b.master_stageid,b.mapseq,a.stage_sequence
                                                                ...
                                                                as masterstagename from sales_stage a,stage_cycle_mapping b,
                                                                sales_cycle c where a.stage_id=b.stage_id
    ...
    public function cyclename_data(){
        try{
                $query=$GLOBALS['$dbFramework']->query("select * from sales_cycle order by id");
                return $query->result();
        }catch (LConnectApplicationException $e){
            $GLOBALS['$logger']->debug('!!!Exception Thrown to Model --- Passing to Controller!!!');
            throw $e;
    ...
    public function get_master_stage($cycleid,$status,$stgseq){
        ...
        try{
               ...
               $query=$GLOBALS['$dbFramework']->query(" SELECT a.master_stageid FROM stage_cycle_mapping a,sales_stage b where
               ...
               if($query->num_rows()>0){
                ...
                if($status=="add"){
                        $que1=$GLOBALS['$dbFramework']->query("select a.id,a.stage_id,a.stage_name,b.remarks,a.stage_sequence from sales_stage a,stage_cycle_mapping b,
                                                master_sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.master_cycleid and
                                                c.master_cycleid='".$mstageid."'
                                                and a.stage_sequence >= (select max(d.mapseq) from sales_stage e,stage_cycle_mapping d,
                                                sales_cycle f where e.stage_id=d.stage_id and d.cycle_id=f.CYCLE_ID and
                        ...
                        if($que1->num_rows()>0){
                                ...
                                $que=$GLOBALS['$dbFramework']->query("select a.id,a.stage_id,a.stage_name,b.remarks,a.stage_sequence from sales_stage a,stage_cycle_mapping b,
                                                master_sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.master_cycleid and
                      ...
                      $que1=$GLOBALS['$dbFramework']->query("select max(e.stage_sequence) as mapseq from sales_stage e,stage_cycle_mapping d,
                                                                sales_cycle f where e.stage_id=d.stage_id and d.cycle_id=f.CYCLE_ID and
                                                                f.CYCLE_ID='".$cycleid."' and d.mapseq is not null;");
                      if($que1->num_rows()>0){
                      ...
                      if($stgseq > $max_mapseq){
                                ...
                                $que1=$GLOBALS['$dbFramework']->query("select a.id,a.stage_id,a.stage_name,b.remarks,a.stage_sequence from sales_stage a,stage_cycle_mapping b,
                                                master_sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.master_cycleid and
                                                c.master_cycleid='".$mstageid."'
                                                and a.stage_sequence <= (select min(d.mapseq) from sales_stage e,stage_cycle_mapping d,
                                                sales_cycle f where e.stage_id=d.stage_id and d.cycle_id=f.CYCLE_ID and
                                                    f.CYCLE_ID='".$cycleid."' and d.mapseq is not null)
                                                order by a.stage_sequence");
    ...
    public function get_master_stage_edit($id,$prevId,$nextId,$activeCycleID){
        try{
              ...
              if($prevId<>0){
                    $que1=$GLOBALS['$dbFramework']->query("select mapseq from stage_cycle_mapping where id=".$prevId.";");
                     if($que1->num_rows()>0){
                            foreach ($que1->result() as $row)
                                  {
                                          $strtpt = $row->mapseq;
                     ...
                     if($nextId<>0){
                            $que1=$GLOBALS['$dbFramework']->query("select mapseq from stage_cycle_mapping where id=".$nextId.";");
                            if($que1->num_rows()>0){
                                foreach ($que1->result() as $row)
                                      {
                                              $endpt=$row->mapseq;
                             ...
                             $que1=$GLOBALS['$dbFramework']->query("select * from sales_cycle where CYCLE_ID='".$activeCycleID."';");
                             if($que1->num_rows()>0){
                                    foreach ($que1->result() as $row)
                                          {
                             ...
                             $que1=$GLOBALS['$dbFramework']->query("select a.stage_id as master_stageid,a.stage_name as masterstagename,a.stage_sequence as mapseq
                                                                      from sales_stage a,stage_cycle_mapping b,
                                                                      master_sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.master_cycleid and
    ...
    public function insert_data($data1,$data2,$stageName,$cycleId,$new_stageid) {
        try{
                $str="";
                $query = $GLOBALS['$dbFramework']->query("select a.stage_name from sales_stage a, stage_cycle_mapping b
    ...
    public function update_data($stage_id,$stage_name,$edit_cycleid,$description,$masterid,$seqid){
        ...
        try{
                $str="";
                $query = $GLOBALS['$dbFramework']->query("select a.stage_name,a.stage_id from sales_stage a, stage_cycle_mapping b
                                                            where a.stage_id=b.stage_id and
                ...
                if($query->num_rows()>0){
                    ...
                    if(strtolower($stage_id1) <> strtolower($stage_id)){
                        ...
                        $str="1";
                        $GLOBALS['$dbFramework']->query("update stage_cycle_mapping set remarks='$description',master_stageid='$masterid',mapseq='$seqid'
                 ...
                 $b=array();
                 $oppquery=$GLOBALS['$dbFramework']->query("select distinct cycle_id from stage_cycle_mapping where cycle_id ='$edit_cycleid'");    // check the opportunity against cycle id
                 if($oppquery->num_rows()>0){
    ...
    public function get_update_data($cycle_id){
        $b=array();
        try{
                  $oppquery=$GLOBALS['$dbFramework']->query("select distinct cycle_id from stage_cycle_mapping where cycle_id ='$cycle_id'");    // check the opportunity against cycle id
                  if($oppquery->num_rows()>0){
    ...
    public function get_cycle(){
        try{
                $que=$GLOBALS['$dbFramework']->query("select c.CYCLE_NAME,c.CYCLE_ID from sales_stage a,stage_cycle_mapping b,
                                                  sales_cycle c where a.stage_id=b.stage_id and b.cycle_id=c.CYCLE_ID
    ...
    public function create_duplicate($from,$to){
        ...
        try{
                    ...
                    $mque=$GLOBALS['$dbFramework']->query("select * from sales_cycle where CYCLE_ID='".$from."'");
                    if($mque->num_rows()>0){
                    ...
                    $mque=$GLOBALS['$dbFramework']->query("select * from sales_cycle where CYCLE_ID='".$to."'");
                    if($mque->num_rows()>0){
                        foreach ($mque->result() as $row)
                        {
                            $fromcycle_mcid1 = $row->MASTERCYCLE_ID;
                                ...
                                if($que1->num_rows()>0){
                                      ...
                                      for($i=0;$i<count($arr);$i++){
                                              ...
                                              $que11=$this->db->query("SELECT b.* FROM stage_cycle_mapping a,sales_stage_attributes b where
                                     ...
                                     $var3 = $GLOBALS['$dbFramework']->insert_batch('sales_stage_attributes',$stg_att_array1);
                                }
