...
$GLOBALS['$log'] = Logger::getLogger('admin_buyerpersonaController');

class admin_buyerpersonaController extends Master_Controller{
    public function __construct(){
        parent::__construct();
        $this->load->helper('url');
        $this->load->model('admin_buyerpersonaModel','buyer');
    }
    public function index(){
        if($this->session->userdata('uid')){
           $this->load->view('admin_buyerpersona_view');
        }else{
    ...
    public function post_data() {
        if($this->session->userdata('uid')){
            try{
                    ...
                    $data = array(
                        'lookup_id' => $buyerpID,
                        'lookup_name' => 'Buyer Persona',
                        'lookup_key' => $buyerpKey,
                        'lookup_value' =>ucfirst(strtolower($buyerpName))
                    );
                    $insert = $this->buyer->insert_data($data,$buyerpName);
                    if($insert==1){
                        $buyer = $this->buyer->view_data();
                        echo json_encode($buyer);
                    }
    ...
    public function update_data(){
        if($this->session->userdata('uid')){
            try{
                  ...
                  $update = $this->buyer->update_data($buyerpID,$data,$buyerpName);
                  if($update==1){
                      $buyer = $this->buyer->view_data();
                      echo json_encode($buyer);
                  }
