...
$GLOBALS['$log'] = Logger::getLogger('admin_industry_hierarchyController');

class admin_industry_hierarchyController extends Master_Controller{
    public function __construct(){
        parent::__construct();
       $this->load->model('admin_industry_hierarchyModel','indus_hierarchy');
    }
    public function index(){
        if($this->session->userdata('uid')){
           $this->load->view('admin_industry_hierarchy_view');
        }else{
    ...
    public function get_lead_source() {
        if($this->session->userdata('uid')){
          try{
                    $source = $this->indus_hierarchy->get_lead_source();
                    $json = array();
                    $hkey11="";
    ...
    public function update_source() {
        if($this->session->userdata('uid')){
           try{
                    ...
                    $update = $this->indus_hierarchy->update_source($source,$hid,$parent_id,$hid1);
                    if($update==true){
	...
	public function post_lead_data() {
        if($this->session->userdata('uid')){
          try{
        			...
        			$hierarchy = $this->indus_hierarchy->get_hierarchy_id();
        			...
        			$insert = $this->indus_hierarchy->insert_hierarchy($data,$source,$parent_id);
        			if($insert==true){
        				$this->get_lead_source();
        			}else{
                        echo "0";
