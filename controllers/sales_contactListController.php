...
class sales_contactListController extends Master_Controller{
	public function __construct(){
		parent::__construct();
		$this->load->helper('url');
		$this->load->library('session');
		$this->load->model('sales_contactListModel','contacts');
	}
	...
	public function index(){
		if($this->session->userdata('uid')){
			try {
				$this->load->view('sales_contacts_view');
			}
			...
			try {
				...
				if ( ! $this->upload->do_upload('userfile'))    {
					...
					if (rename($old_path, $new_path)) {
						...
						$insert = $this->contacts->edit_contact($data, $path);
						if ($insert == TRUE) {
