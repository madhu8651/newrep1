...
$GLOBALS['$log'] = Logger::getLogger('admin_salescycle_parameterController');

class admin_salescycle_parameterController extends Master_Controller{
    public function __construct(){
        parent::__construct();
        $this->load->helper('url');
        $this->load->library('session');
        //$this->output->enable_profiler(true);
        $this->load->model('admin_salescycle_parameterModel','cycle');
    }
    public function index(){
        if($this->session->userdata('uid')){
         $this->load->view('admin_salescycle_parameterView');
        }else{
    ...
    public function teams(){
        if($this->session->userdata('uid')){
          try{
                ...
                $teams = $this->cycle->team_data($deptid);
    ...
    public function product(){
        ...
        if($this->session->userdata('uid')){
            try{
                  $industry_data = $this->cycle->view_productdata();
                  $json = array();
                  $hkey11="";
    ...
    public function industry(){
        if($this->session->userdata('uid')){
             try{
                  $industry_data = $this->cycle->view_industrydata();
                  $json = array();
                  $hkey11="";
                  for($i=0;$i<count($industry_data);$i++){
    ...
    public function locations(){
        if($this->session->userdata('uid')){
            try{
                  $location_data = $this->cycle->view_locationdata();
                  $json = array();
                  $hkey11="";
    ...
    public function get_cycle(){
        if($this->session->userdata('uid')){
          try{
                $cyclename = $this->cycle->cycle_data();
     ...
     public function post_data(){
        if($this->session->userdata('uid')){
            try{
                      ...
                      $delDuplicate=$this->cycle->delDuplicate($listCombId);
                      $insert = $this->cycle->insert_data($maindata1);
        ...
        public function get_productdata_edit(){
          if($this->session->userdata('uid')){
            try{
                    ...
                    $industry_edit=$this->cycle->product_edit($paramid);
    ...
    public function get_industry_edit(){
         if($this->session->userdata('uid')){
           try{
                    ...
                    $industry_edit=$this->cycle->industry_edit($paramid);
                    echo json_encode($industry_edit);
           }catch (LConnectApplicationException $e)  {
        					$GLOBALS['$logger']->debug('!!!Exception Thrown to Controller --- Passing as JSON to View!!!');
        					$errorArray = array(
    ...
    public function get_business_edit(){
          if($this->session->userdata('uid')){
            try{
                    ...
                    $paramid=$data->paramid;
                    $business_edit=$this->cycle->business_edit($paramid);
    ...
    public function update_data(){
          if($this->session->userdata('uid')){
            try{
                    ...
                    $delDuplicate=$this->cycle->delDuplicate($listCombId);
                    $insert = $this->cycle->update_data($maindata1,$hid_parameterid);
                    echo json_encode($insert);
            }catch (LConnectApplicationException $e)  {
        					$GLOBALS['$logger']->debug('!!!Exception Thrown to Controller --- Passing as JSON to View!!!');
        					$errorArray = array(
     ...
     public function checkDuplicate($status){
           ...
           if($this->session->userdata('uid')){
                try{
                          ...
                          $hid_parameterid=$data->hid_parameterid;
                          $checkDuplicate=$this->cycle->checkDuplicate($listCombId,$status,$hid_parameterid);
